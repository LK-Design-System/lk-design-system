...
assert(expectedSlugs.every((slug) => content.foundations.some((item) => item.slug === slug)), 'Foundation inventory does not match the required 16 areas.');

const requiredArrays = [
  'principles', 'semanticModel', 'selectionCriteria', 'quantitativeRules', 'doDont',
  'exceptions', 'accessibility', 'internationalization', 'examples', 'tokens', 'apis',
];
for (const foundation of content.foundations) {
  ...
  assert(Array.isArray(foundation.sources), `${foundation.slug}: sources must be an array.`);
}
