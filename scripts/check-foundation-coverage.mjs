...
for (const [slug, title] of expected) {
  ...
  assert(foundation.sources.some(({ url }) => url?.startsWith('https://seed-design.io/')), `${slug}: SEED benchmark trace is required.`);
...
for (const [slug, title] of expected) {
  ...
  assert(
    storySource.includes(`foundationGuideStory('${slug}'`),
    `Storybook Foundation page does not render the complete structured guide: ${slug}`,
  );
}
...
console.log(`Validated Foundation completeness: ${expected.length}/15 structured guides, ${expected.length}/15 public Storybook pages, required depth and generated surfaces present.`);
