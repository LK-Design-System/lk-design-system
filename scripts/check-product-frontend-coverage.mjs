...
const portalConsumerScanPath = 'docs/references/package-split/consumers/portal.json';
const roboticsExternalSurfacePath = 'docs/references/package-split/ROBOTICS_EXTERNAL_SURFACE.json';
const requiredRepositories = {
  deviceops: 'LK-ROBOTICS/lk_deviceops',
  visionops: 'LK-ROBOTICS/lk_visionops',
...
const portalConsumerScan = JSON.parse(await readFile(path.join(root, portalConsumerScanPath), 'utf8'));
...
assert(!/LK Context Hub|lk_context_hub|context-hub/.test(source), 'Current product workflow audit must use the LK Portal identity; historical package-split evidence is maintained separately.');
assert(!/LK Context Hub|lk_context_hub|context-hub/.test(documentation), 'Current product workflow documentation must use the LK Portal identity; historical package-split evidence is maintained separately.');
...
assert(audit.repositories.length === Object.keys(requiredRepositories).length, `Audit must contain exactly ${Object.keys(requiredRepositories).length} repositories.`);
...
for (const repository of audit.repositories) {
  ...
  assert(requiredRepositories[repository.id] === repository.repository, `${repository.id} must identify ${requiredRepositories[repository.id] || 'a required product repository'}.`);
...
for (const [repositoryId, repositoryName] of Object.entries(requiredRepositories)) {
  assert(repositoryIds.has(repositoryId), `Missing required product id: ${repositoryId}`);
  assert(repositoryNames.has(repositoryName), `Missing required repository: ${repositoryName}`);
}

const portalRepository = audit.repositories.find((repository) => repository.id === 'portal');
assert(portalRepository, 'LK Portal repository record is required.');
assert(portalConsumerScan.kind === 'lds-consumer-import-scan', 'LK Portal consumer evidence must be generated by scan-package-consumer.mjs.');
assert(portalConsumerScan.scanner === 'scripts/scan-package-consumer.mjs', 'LK Portal consumer evidence must record its scanner.');
assert(portalConsumerScan.repository === portalRepository.repository, 'LK Portal consumer evidence repository must match the current product audit.');
assert(portalConsumerScan.commit === portalRepository.commit, 'LK Portal consumer evidence commit must match the current product audit.');
assert(portalConsumerScan.frontendRoot === portalRepository.frontendRoot, 'LK Portal consumer evidence frontend root must match the current product audit.');
assert(/^[0-9a-f]{64}$/.test(portalConsumerScan.sourceTreeSha256), 'LK Portal consumer evidence requires a source-tree hash.');
assert(Number.isInteger(portalConsumerScan.sourceFileCount) && portalConsumerScan.sourceFileCount > 0, 'LK Portal consumer evidence requires a positive source-file count.');
for (const key of ['dependencyDeclarations', 'aggregateRootImports', 'layerSubpathImports', 'componentDeepImports', 'stylesheetImports', 'assetFilesystemPaths', 'lds3dPackageImports', 'cjsRequireOccurrences']) {
  assert(Number.isInteger(portalConsumerScan.currentUsage?.[key]) && portalConsumerScan.currentUsage[key] >= 0, `LK Portal consumer evidence has invalid ${key} usage.`);
  assert(Array.isArray(portalConsumerScan.matches?.[key]), `LK Portal consumer evidence has invalid ${key} matches.`);
  assert(portalConsumerScan.currentUsage[key] === portalConsumerScan.matches[key].length, `LK Portal consumer evidence ${key} count does not match its recorded occurrences.`);
}
