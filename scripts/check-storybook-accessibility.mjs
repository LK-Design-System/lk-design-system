...
async function main() {
  ...
  try {
    ...
    // The component decision guides render on Docs tabs, not on story canvases, so the story
    // loop above never sees them. Axe is scoped to the guide itself: Storybook's own docs
    // chrome is not ours to fix, and auditing it would only produce unfixable noise.
    for (const doc of docsPages) {
      await page.goto(`${origin}/iframe.html?id=${encodeURIComponent(doc.id)}&viewMode=docs`, {
        waitUntil: 'domcontentloaded',
        timeout: 45000,
      });
      // Component pages carry a component guide; Foundation pages carry a foundation guide.
      const guideReady = await page
        .waitForSelector('[data-component-guide], [data-foundation-guide]', { state: 'attached', timeout: 30000 })
        .then(() => true)
        .catch(() => false);
      if (!guideReady) {
        failures.push(`${doc.title}: Docs page never rendered its component decision guide.`);
        continue;
      }
      docsPagesChecked += 1;
      await page.evaluate(() => document.fonts?.ready).catch(() => {});
      await page.addScriptTag({ content: axe.source });
      const docsResults = await page.evaluate(async (rules) => {
        const guide = document.querySelector('[data-component-guide], [data-foundation-guide]');
        for (let attempt = 0; attempt < 50; attempt += 1) {
          try {
            return await window.axe.run(guide, { runOnly: { type: 'rule', values: rules } });
          } catch (error) {
            if (!String(error?.message || error).includes('already running') || attempt === 49) throw error;
            await new Promise((resolve) => setTimeout(resolve, 100));
          }
      ...
      for (const violation of docsResults.violations) {
        for (const node of violation.nodes) {
          failures.push(`${doc.title} (Docs): axe ${violation.id} (${violation.impact ?? 'unknown'}) ${node.target.join(' ')} — ${node.failureSummary}`);
        }
      }
    }
  } finally {
  ...
  assert(
    docsPagesChecked === docsPages.length,
    `Every Docs page must render an auditable component guide: ${docsPagesChecked}/${docsPages.length}.`,
  );

  console.log(`Validated Storybook accessibility guard: ${stories.length} implementation stories, ${playFunctionStories} completed play functions, ${axeCheckedStories} Axe-guarded stories, ${docsPagesChecked} Axe-guarded Docs guides, ${normalizedTargets.length} ratcheted undersized target signatures, 0 violations, 0 missing names, 0 implicit button types, 0 console errors.`);
}
