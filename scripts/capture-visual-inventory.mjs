  ...
  try {
    ...
    await page.waitForFunction(
      (expectedId) => {
        ...
        if (rootRect.width <= 0 || rootRect.height <= 0 || root.children.length === 0) return false;
...
async function main() {
  ...
  try {
    ...
    for (const selected of componentCards) {
      ...
      await gotoStoryReady(page, storyUrl(origin, pair.primaryStory.id), pair.primaryStory.id);
      await page.evaluate(async () => {
        if (document.fonts?.ready) await document.fonts.ready;
      });
