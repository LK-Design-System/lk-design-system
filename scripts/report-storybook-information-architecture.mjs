...
const indexPath = process.env.STORYBOOK_INDEX_PATH
  ? path.resolve(process.env.STORYBOOK_INDEX_PATH)
...
const docsDedupContractPath = path.join(root, 'docs', 'references', 'quality', 'STORYBOOK_DOCS_DEDUP_CONTRACT.json');
...
const DOCUMENT_TYPE_CONTRACTS = {
  Pattern: 'problem → authority/applicability → primary decision → component composition → states/failure → accessibility',
};
...
const PAGE_OWNER_OVERRIDES = {
  ...
  'LDS Core/Components/Content/Text Primitives': ['Blockquote', 'Code', 'Kbd', 'SourceTag'],
  'LDS Core/Components/Content/Media Patterns': ['Thumbnail'],
  ...
  'LDS Theme/Brand/LK ROBOTICS Logo': ['Lockup'],
...
function validateDocsDedupIndex(index, contract) {
  const entries = Object.values(index.entries || {});
  const stories = entries.filter((entry) => entry.type === 'story').sort((a, b) => a.id.localeCompare(b.id));
  const docs = entries.filter((entry) => entry.type === 'docs');
  const visibility = stories.map((story) => ({
    id: story.id,
    visibility: story.tags?.includes('dev') ? 'public' : 'hidden',
  }));
  const publicStories = visibility.filter((story) => story.visibility === 'public').length;
  const expected = contract.expected;

  assert(new Set(stories.map((story) => story.id)).size === stories.length, 'Built Storybook index contains duplicate story IDs.');
  assert(stories.length === expected.stories, `Built Storybook must retain ${expected.stories} stories; found ${stories.length}.`);
  assert(docs.length === expected.docs, `Built Storybook must expose ${expected.docs} Docs routes; found ${docs.length}.`);
  assert(publicStories === expected.publicStories, `Built Storybook public visibility changed: expected ${expected.publicStories}, found ${publicStories}.`);
  assert(stories.length - publicStories === expected.hiddenStories, `Built Storybook hidden visibility changed: expected ${expected.hiddenStories}, found ${stories.length - publicStories}.`);
  assert(
    sha256(stories.map((story) => story.id).join('\n')) === expected.storyIdsSha256,
    'Built Storybook story ID census changed during Docs deduplication.',
  );
  assert(
    sha256(visibility.map((story) => `${story.id}:${story.visibility}`).join('\n')) === expected.visibilitySha256,
    'Built Storybook public/hidden visibility changed during Docs deduplication.',
  );
}
...
function preserveManualFields(page, previousPage) {
  ...
  for (const key of [
    ...
    'reviewNote',
    // A page whose subject has no renderable specimen enters through its Docs tab instead of
    // a canvas story. That is a reviewed decision, so it survives regeneration.
    'documentationOnly',
  ]) {
...
async function buildAudit(previous) {
  ...
  const docsDedupContract = JSON.parse(await read(docsDedupContractPath));
  validateDocsDedupIndex(index, docsDedupContract);
  ...
  for (const [importPath, fileStories] of [...byImport].sort(([a], [b]) => a.localeCompare(b))) {
    ...
    const isPattern = fileStories[0].title.startsWith(PATTERN_TITLE_PREFIX);
    ...
    const auditedStories = orderedStories.map((story) => {
      ...
      return {
        ...
        hasStoryDescription:
          ...
          // Helpers that attach a story description on the caller's behalf.
          /\b(foundationGuideStory|foundationSpecimenStory)\s*\(/.test(usage.text),
    ...
    const page = {
      ...
      descriptionEvidence: {
        ...
        ...(isPattern ? { patternGuide: /\bpatternGuide\s*:\s*[A-Za-z_$][\w$]*/.test(source) } : {}),
      },
      missingGuidance: [
        ...
        isPattern && !/\bpatternGuide\s*:\s*[A-Za-z_$][\w$]*/.test(source) ? 'pattern-guide-contract' : null,
  ...
  return {
    ...
    documentTypeDescriptionContracts: DOCUMENT_TYPE_CONTRACTS,
    namingContract: {
...
if (update) {
  ...
  assert(
    audit.documentTypeDescriptionContracts.Pattern === DOCUMENT_TYPE_CONTRACTS.Pattern,
    'Missing description contract for Pattern pages.',
  );
  ...
  for (const page of audit.pages) {
    ...
    if (page.title.startsWith(PATTERN_TITLE_PREFIX)) {
      assert(page.family === 'Patterns', `${page.title} must stay inside the LDS Core/Patterns family.`);
      assert(page.descriptionEvidence.patternGuide, `${page.title} must connect to the shared PatternGuide contract.`);
    }
    assert(
