...
import { HAZARD_GLYPH_PATHS, HAZARD_GLYPH_FIT } from '../components/robotics/_HazardGlyph.js';
import { FACILITY_GLYPH_PATHS } from '../components/robotics/_FacilityGlyph.js';
...
const meta = {
  ...
  parameters: {
    storyGuide: {
      ...
      description:
        '해저드 마커가 severity 색 핀 배지 위에 찍는 위험물 종류 글리프입니다. 계단·단차(낙하)·충돌 위험물 세 실루엣은 내부 원자 _HazardGlyph가 단일 소스로 소유하고(HAZARD_GLYPH_PATHS), 경사로는 같은 물리적 대상이 통과 설비일 수도 회피 대상일 수도 있어 Facility Glyph 원자를 공유합니다. 이 페이지는 그 path를 fit 변환과 함께 그대로 렌더해 배지 크기에서 서로 구분되는지 보이고, play-test로 렌더된 path가 상수와 일치함을 단언합니다. 공개 API가 아닌 내부 글리프 모듈입니다.',
    },
    docs: {
      description: {
        component:
