  ...
  return (
    <div role="list" aria-label="로봇 목록" style={{ display: 'grid', minWidth: 0 }}>
      <style>
        ...
        .lk-story-primary-item[aria-current="true"] { background: var(--color-semantic-primary-surface-normal); }`}
      </style>
      {ITEMS.map((item) => (
...
export const InlineDetail = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const selectedStyle = getComputedStyle(second);
    if (selectedStyle.backgroundColor === 'transparent' || selectedStyle.boxShadow !== 'none') {
      throw new Error('Primary-detail selection must use a quiet surface without a navigation-style edge indicator.');
    }
