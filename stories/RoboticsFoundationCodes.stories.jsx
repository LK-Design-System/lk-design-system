...
const meta = {
  ...
  parameters: {
    storyGuide: {
      ...
      description:
        '지도 위에서 언어에 상관없이 읽히는 짧은 코드를 내부 모듈 _navigationEncoding이 단일 소스로 소유합니다: 웨이포인트 역할 코드 ROLE_CODE(H·T·P·C)와 지점 주석 코드 ANNOTATION_CODE(dock·clean·disp 등). 마커 배지와 지도 범례가 같은 이 코드 집합을 함께 디코드하므로 지도와 범례가 서로 어긋나지 않습니다. 이 페이지는 두 레지스트리를 그대로 렌더해 각 칩의 텍스트가 소스 코드 값과 일치함을 회귀로 못 박습니다. 공개 API가 아닌 내부 코드 레지스트리입니다.',
    },
    docs: {
      description: {
        component:
