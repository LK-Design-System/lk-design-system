...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: '사람·팀·기관을 식별하는 Avatar와 여러 대상을 겹쳐 보여주는 AvatarGroup 패턴입니다. 식별 콘텐츠라는 의미에 맞춰 Content 그룹에 둡니다.',
      },
    },
  },
};
...
function MatrixSection({ title, children }) {
  return (
    ...
    >
      <h2 style={{ margin: 0, fontSize: 16, lineHeight: 1.3, color: 'var(--color-semantic-label-normal)' }}>{title}</h2>
      {children}
    </section>
  );
}
...
function AvatarResourceComparison() {
  return (
    <section style={{ display: 'grid', gap: 'var(--space-3)' }}>
      <h2 style={{ margin: 0, fontSize: 16, lineHeight: 1.3, color: 'var(--color-semantic-label-normal)' }}>리소스 패리티</h2>
