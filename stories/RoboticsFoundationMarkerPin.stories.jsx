...
import { NAV_PIN } from '../components/robotics/_navigationVocabulary.js';
...
// shape), and the contrasting circular waypoint origin sits alongside to show
...
// contrasting circle marker, not a pin.
...
function PinFamily() {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: 10 }}>
      ...
      <Tile label="웨이포인트 원점 (원형 마커 · 핀 아님)" mono="circle">
        <svg width={88} height={92} viewBox="0 0 56 52" aria-hidden="true" style={{ display: 'block' }}>
...
function MarkerPinCatalog() {
  return (
    <main data-marker-pin-catalog style={{ width: 'min(880px, 100%)', display: 'grid', gap: 16 }}>
      <Card
        title="핀 패밀리"
        hint="설비·해저드 마커는 같은 map-pin 실루엣(NAV_PIN)을 공유하고, 형태가 아니라 accent·severity 색과 knockout 글리프로만 구분됩니다. 그림자와 focus·selection 링도 같은 실루엣을 scale()로 따라 그립니다. 웨이포인트 원점은 대비를 위해 나란히 둔 원형 마커로, 핀이 아닙니다. 배지 글리프 세트는 State Badge, 핀 위 knockout 글리프는 Facility·Hazard Glyph 페이지를 참고하세요."
      >
        <PinFamily />
      </Card>
    </main>
...
const meta = {
  ...
  parameters: {
    storyGuide: {
      ...
      description:
        '설비 전이·해저드 마커가 한 지도에서 하나의 마커 패밀리로 읽히도록, 두 마커는 같은 map-pin 실루엣(NAV_PIN)을 몸통으로 공유하고 severity·accent 색과 knockout 글리프로만 구분됩니다. 이 페이지는 그 몸통을 실제 FacilityTransition·HazardMarker로 그대로 렌더하고, 대비되는 원형 웨이포인트 원점을 나란히 놓아 무엇이 핀이고 무엇이 아닌지 보입니다. NAV_PIN의 path·그림자·focus/selection 링 기하는 내부 모듈 _navigationVocabulary가 단일 소스로 소유하며, play-test는 실제 해저드 마커가 NAV_PIN.path 실루엣을 소비함을 단언합니다. 공개 API가 아닌 내부 모듈입니다.',
    },
    docs: {
      description: {
        component:
          '설비·해저드 마커가 공유하는 map-pin 몸통(NAV_PIN)을 실제 마커로 렌더해 문서화·회귀합니다. 두 마커는 같은 실루엣을 쓰고 색·knockout 글리프로만 구분되며, 웨이포인트 원점은 대비되는 원형 마커입니다. NAV_PIN 기하는 내부 모듈 _navigationVocabulary가 소유하고, 핀 위에 얹히는 상태·설비·해저드 글리프 세트는 각 글리프 카탈로그 페이지가 별도로 다룹니다. 공개 API가 아닌 내부 모듈입니다.',
      },
    },
  },
};
...
export const Overview = {
  ...
  parameters: storyDescription(
    '공유 map-pin 몸통을 실제 마커 위에서 봅니다. 설비·해저드 핀은 같은 NAV_PIN 실루엣을 쓰고, 웨이포인트 원점은 대비되는 원형 마커입니다. play-test가 실제 해저드 마커의 실루엣 d가 NAV_PIN.path와 일치함을 단언하므로 이 페이지가 곧 몸통 기하의 회귀 기준입니다.',
  ),
