...
export const ListingCards = {
  ...
  render: () => (
    <main style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 'var(--space-4)', maxWidth: 820 }}>
      ...
      <ListingCard
        headingLevel={2}
        image={cover}
        title="2026 하반기 로봇 안전 교육"
        meta={[
          { icon: 'calendar', label: '2026.07.01 ~ 2026.12.31' },
          { icon: 'location', label: '서울 송파구' },
          { icon: 'tag', label: '교육, 안전, 현장 운영' },
        ]}
        status="신청 마감"
        statusTone="neutral"
        href="/events/robot-safety"
      />
    </main>
  ),
  play: async ({ canvasElement }) => {
    ...
    if (open.getAttribute('aria-label') !== '정기 점검 운영 캠페인. 진행중') {
      throw new Error('문자열 status는 링크 접근 이름 끝에 "제목. 상태"로 합성되어야 합니다.');
    }
    if (closed.getAttribute('aria-label') !== '2026 하반기 로봇 안전 교육. 신청 마감') {
...
export const NoCover = {
  ...
  render: () => (
    <main style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 'var(--space-4)', maxWidth: 560 }}>
      ...
      <ListingCard
        headingLevel={2}
        title="2026 로봇 운영 효율 리포트"
        meta={[{ icon: 'tag', label: '운영 분석, 설비 데이터' }]}
        status="상시"
        statusTone="neutral"
        href="/resources/operations-report"
      />
    </main>
  ),
};
  ...
  render: () => (
    <main data-testid="listing-narrow" style={{ width: 320, maxWidth: '100%' }}>
      <ListingCard
        headingLevel={2}
        image={cover}
        title="2026 현장 설비 운영 교육 안내 (5~6월 과정)"
        meta={[
          { icon: 'calendar', label: '2026.03.03 ~ 2026.12.31 · 온라인' },
          { icon: 'tag', label: '교육, 설비 운영, 안전 점검' },
        ]}
        status="진행중"
        statusTone="positive"
        href="/events/ax"
