...
export const Metrics = {
  ...
  render: () => (
    <main style={metricGridStyle}>
      <MetricCard
        ...
        caption="전체 26대"
        lastUpdated="2분 전"
        action={<Link href="#equipment-detail">상세 보기</Link>}
  ...
  play: async ({ canvasElement }) => {
    ...
    const action = cards[0].querySelector('[data-metric-action] a');
    const expectedActionSize = getComputedStyle(cards[0]).getPropertyValue('--label2-size').trim();
    const expectedActionLine = getComputedStyle(cards[0]).getPropertyValue('--label2-line').trim();
    if (!action || getComputedStyle(action).fontSize !== expectedActionSize || getComputedStyle(action).lineHeight !== expectedActionLine) {
      throw new Error('MetricCard footer links must use the compact label2 action type.');
    }
    const footerBottomGaps = [...cards].map((card) => {
      const footer = card.querySelector('[data-metric-footer]');
      return footer ? card.getBoundingClientRect().bottom - footer.getBoundingClientRect().bottom : 0;
    });
    if (Math.max(...footerBottomGaps) - Math.min(...footerBottomGaps) > 3) {
      throw new Error('An optional footer action must not create excess bottom space in sibling MetricCards.');
    }
  },
};
