...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: '짧은 보조 설명을 trigger에 연결하고 크기·위치·화살표 정렬·단축키를 표현하는 LDS Tooltip 패턴입니다.',
      },
    },
  },
};
...
export const TooltipPatterns = {
  ...
  render: () => (
    <main style={{ display: 'grid', gap: 'var(--space-6)', width: '100%', minWidth: 0, maxWidth: 900 }}>
      <Section title="기본 사용">
      ...
      <Section title="크기와 단축키">
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-20)', minHeight: 142, paddingTop: 'var(--space-12)', flexWrap: 'wrap' }}>
  ...
  play: async ({ canvasElement }) => {
    /* The rounded body and pointer are one SVG path. The surface still paints
       outside the bubble's layout box, so the bubble remains non-scrolling and
       long content keeps its independent inner scroll wrapper. */
    const cases = [
      ['tooltip-top', 'top'],
      ['tooltip-left', 'left'],
      ['tooltip-small', 'top'],
    ...
    for (const [testId, expectedPlacement] of cases) {
      ...
      const surface = bubble.querySelector(':scope > svg[data-lds-tooltip-surface]');
      if (!surface) throw new Error(`${testId} must render its unified tooltip surface.`);
      const paths = surface.querySelectorAll(':scope > path');
      if (paths.length !== 1) {
        throw new Error(`${testId}: the rounded body and pointer must be one SVG path, received ${paths.length}.`);
      }
      if (bubble.querySelector(':scope > span[aria-hidden="true"]')) {
        throw new Error(`${testId}: a detached arrow element reintroduces the zoom seam.`);
      }

      // The path itself must extend toward the trigger by the declared pointer
      // height. This covers the actual painted geometry instead of a second DOM
      // box whose 1px overlap could separate at fractional coordinates.
      ...
      const overhang = expectedPlacement === 'left'
        ? pathRect.right - bubbleRect.right
        : pathRect.bottom - bubbleRect.bottom;
      if (Math.abs(overhang - arrowHeight) > 0.5) {
        throw new Error(`${testId}: the unified pointer overhang is ${overhang}px, expected ${arrowHeight}px.`);
      }
    }
  },
};
