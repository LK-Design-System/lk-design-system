...
function DockPanel({
  ...
  const [resizeFocused, setResizeFocused] = _react2.default.useState(false);
  const [isResizing, setIsResizing] = _react2.default.useState(false);
  const isOpen = controlled ? open : internalOpen;
  const isLeft = side === "left";
  const panelId = _react2.default.useId();
  const titleId = `${panelId}-title`;
  const handleRef = _react2.default.useRef(null);
  const dragRef = _react2.default.useRef(null);
  const numericWidth = toNumber(width);
  const minPanelWidth = _nullishCoalesce(toNumber(minWidth), () => ( DEFAULT_MIN_WIDTH));
  const maxPanelWidth = Math.max(minPanelWidth, _nullishCoalesce(toNumber(maxWidth), () => ( DEFAULT_MAX_WIDTH)));
  const canResize = Boolean(resizable && numericWidth !== void 0);
  const boundedInitialWidth = clamp(_nullishCoalesce(numericWidth, () => ( DEFAULT_WIDTH)), minPanelWidth, maxPanelWidth);
  const [resizedWidth, setResizedWidth] = _react2.default.useState(boundedInitialWidth);
          ...
          children: [
            /* @__PURE__ */ _jsxruntime.jsx.call(void 0,
              "button",
              {
                ...
                children: /* @__PURE__ */ _jsxruntime.jsx.call(void 0, _chunkX5XHQEI5cjs.Icon, { name: getHandleIcon(side, isOpen), size: 14, "aria-hidden": "true" })
              }
            ),
...
//# sourceMappingURL=chunk-23ETEWGR.cjs.map