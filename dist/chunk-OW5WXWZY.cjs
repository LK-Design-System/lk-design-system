...
function routeProgressGeometry(points, fraction, explicitPosition, viewportScale = 1) {
  ...
  const gapDistance = _chunkAZGN4HJOcjs.NAV_PROGRESS_HEAD.futureGap / scale;
  const setbackDistance = _chunkAZGN4HJOcjs.NAV_PROGRESS_HEAD.tipSetback / scale;
...
function trajectoryProgressGeometry(points, pointIndex, viewportScale = 1) {
  ...
  const headVisible = fullPrefix.length >= 2;
  const { paintedPrefix, tipSetbackPx } = applyTipSetback(fullPrefix, _chunkAZGN4HJOcjs.NAV_PROGRESS_HEAD.tipSetback / scale, scale);
  return {
    point,
    angle: Math.atan2(tangent.dy, tangent.dx) * 180 / Math.PI,
    prefixPoints: paintedPrefix,
    suffixPoints: suffixFrom(points, tipDistance + (headVisible ? _chunkAZGN4HJOcjs.NAV_PROGRESS_HEAD.futureGap / scale : 0)),
    headVisible,
    tipSetbackPx
  };
}
function NavigationProgressHeadDefs({
  ...
  surface,
  inverseScale,
  tipSetbackPx = _chunkAZGN4HJOcjs.NAV_PROGRESS_HEAD.tipSetback
}) {
  return _react2.default.createElement(
    ...
    _react2.default.createElement("marker", {
      id: `${idPrefix}-head`,
      viewBox: _chunkAZGN4HJOcjs.NAV_PROGRESS_HEAD.viewBox,
      ...
      refX: _chunkAZGN4HJOcjs.NAV_PROGRESS_HEAD.refX - tipSetbackPx,
      refY: _chunkAZGN4HJOcjs.NAV_PROGRESS_HEAD.refY,
      markerWidth: _chunkAZGN4HJOcjs.NAV_PROGRESS_HEAD.width * inverseScale,
      markerHeight: _chunkAZGN4HJOcjs.NAV_PROGRESS_HEAD.height * inverseScale,
      markerUnits: "userSpaceOnUse",
      orient: "auto",
      overflow: "visible"
    }, _react2.default.createElement("path", {
      "data-navigation-progress-head-definition": "head",
      d: _chunkAZGN4HJOcjs.NAV_PROGRESS_HEAD.path,
      fill: tone,
      stroke: surface,
      strokeWidth: _chunkAZGN4HJOcjs.NAV_PROGRESS_HEAD.outlineWidth,
      strokeLinejoin: "round"
    }))
  );
}
function ProgressHeadObstacle({ obstacle, id, point, angle, inverseScale, dataPrefix }) {
  const bounds = _chunkAZGN4HJOcjs.NAV_PROGRESS_HEAD.obstacle;
...
//# sourceMappingURL=chunk-OW5WXWZY.cjs.map