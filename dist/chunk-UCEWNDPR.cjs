...
function routeProgressGeometry(points, fraction, explicitPosition, viewportScale = 1, { suppressHead = false } = {}) {
  ...
  const gapDistance = _chunkYXXTXTP5cjs.NAV_PROGRESS_HEAD.futureGap / scale;
  const setbackDistance = _chunkYXXTXTP5cjs.NAV_PROGRESS_HEAD.tipSetback / scale;
...
function trajectoryProgressGeometry(points, pointIndex, viewportScale = 1, { suppressHead = false } = {}) {
  ...
  const tangent = _nullishCoalesce(incoming, () => ( outgoing));
  if (!tangent) return void 0;
  const scale = Number.isFinite(viewportScale) && viewportScale > 0 ? viewportScale : 1;
  const tipDistance = polylineLength(fullPrefix);
  const headVisible = fullPrefix.length >= 2 && !suppressHead;
  const { paintedPrefix, tipSetbackPx } = headVisible ? applyTipSetback(fullPrefix, _chunkYXXTXTP5cjs.NAV_PROGRESS_HEAD.tipSetback / scale, scale) : { paintedPrefix: fullPrefix, tipSetbackPx: 0 };
  return {
    point,
    angle: Math.atan2(tangent.dy, tangent.dx) * 180 / Math.PI,
    prefixPoints: paintedPrefix,
    suffixPoints: suffixFrom(points, tipDistance + (headVisible ? _chunkYXXTXTP5cjs.NAV_PROGRESS_HEAD.futureGap / scale : 0)),
    headVisible,
    tipSetbackPx
  };
}
function NavigationProgressHeadDefs({
  ...
  surface,
  inverseScale,
  tipSetbackPx = _chunkYXXTXTP5cjs.NAV_PROGRESS_HEAD.tipSetback
}) {
  return _react2.default.createElement(
    ...
    _react2.default.createElement("marker", {
      id: `${idPrefix}-head`,
      viewBox: _chunkYXXTXTP5cjs.NAV_PROGRESS_HEAD.viewBox,
      ...
      refX: _chunkYXXTXTP5cjs.NAV_PROGRESS_HEAD.refX - tipSetbackPx,
      refY: _chunkYXXTXTP5cjs.NAV_PROGRESS_HEAD.refY,
      markerWidth: _chunkYXXTXTP5cjs.NAV_PROGRESS_HEAD.width * inverseScale,
      markerHeight: _chunkYXXTXTP5cjs.NAV_PROGRESS_HEAD.height * inverseScale,
      markerUnits: "userSpaceOnUse",
      orient: "auto",
      overflow: "visible"
    }, _react2.default.createElement("path", {
      "data-navigation-progress-head-definition": "head",
      d: _chunkYXXTXTP5cjs.NAV_PROGRESS_HEAD.path,
      fill: tone,
      stroke: surface,
      strokeWidth: _chunkYXXTXTP5cjs.NAV_PROGRESS_HEAD.outlineWidth,
      strokeLinejoin: "round"
    }))
  );
}
function ProgressHeadObstacle({ obstacle, id, point, angle, inverseScale, dataPrefix }) {
  const bounds = _chunkYXXTXTP5cjs.NAV_PROGRESS_HEAD.obstacle;
...
exports.routeProgressGeometry = routeProgressGeometry; exports.trajectoryProgressGeometry = trajectoryProgressGeometry; exports.NavigationProgressHeadDefs = NavigationProgressHeadDefs; exports.ProgressHeadObstacle = ProgressHeadObstacle;
//# sourceMappingURL=chunk-UCEWNDPR.cjs.map