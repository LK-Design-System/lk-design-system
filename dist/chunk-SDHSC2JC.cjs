...
function isAvailable(element) {
  ...
  const style = _optionalChain([view, 'optionalAccess', _8 => _8.getComputedStyle, 'optionalCall', _9 => _9(element)]);
  return _optionalChain([style, 'optionalAccess', _10 => _10.display]) !== "none" && _optionalChain([style, 'optionalAccess', _11 => _11.visibility]) !== "hidden";
}
function getFocusableElements(dialog) {
  return Array.from(_nullishCoalesce(_optionalChain([dialog, 'optionalAccess', _12 => _12.querySelectorAll, 'call', _13 => _13(FOCUSABLE_SELECTOR)]), () => ( []))).filter((element) => element.tabIndex >= 0 && isAvailable(element));
}
function focusDialogStart(dialog, initialFocusRef) {
  if (!dialog) return;
  const requested = _optionalChain([initialFocusRef, 'optionalAccess', _14 => _14.current]);
...
function useDialogFocus({
  ...
  useSafeLayoutEffect(() => {
    ...
    const ownerDocument = _nullishCoalesce(_optionalChain([dialog, 'optionalAccess', _15 => _15.ownerDocument]), () => ( document));
    ...
    const onKeyDown = (event) => {
      ...
      if (focusables.length === 0) {
        event.preventDefault();
        _optionalChain([currentDialog, 'optionalAccess', _16 => _16.focus, 'call', _17 => _17({ preventScroll: true })]);
        return;
      }
    ...
    return () => {
      ...
      const requestedReturn = _optionalChain([optionsRef, 'access', _18 => _18.current, 'access', _19 => _19.returnFocusRef, 'optionalAccess', _20 => _20.current]);
      const returnTarget = isAvailable(requestedReturn) ? requestedReturn : previouslyFocused;
      if (nextOverlay && isAvailable(returnTarget) && _optionalChain([nextOverlay, 'access', _21 => _21.dialogRef, 'access', _22 => _22.current, 'optionalAccess', _23 => _23.contains, 'call', _24 => _24(returnTarget)])) {
...
//# sourceMappingURL=chunk-SDHSC2JC.cjs.map