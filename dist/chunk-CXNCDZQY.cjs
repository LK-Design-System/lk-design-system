...
// packages/core/dist/chunk-GITWNP6T.js
...
function insetCollisionRect(view, boundary, padding) {
  ...
  const source = _nullishCoalesce(_optionalChain([boundary, 'optionalAccess', _21 => _21.getBoundingClientRect, 'call', _22 => _22()]), () => ( viewport));
...
function useControllableOpen({ open, defaultOpen = false, onOpenChange }) {
  ...
  const setVisible = _react2.default.useCallback((nextValue) => {
    ...
    if (next !== visible) _optionalChain([onOpenChange, 'optionalCall', _23 => _23(next)]);
...
function useLightDismiss({
  ...
  _react2.default.useEffect(() => {
    ...
    releaseFocusLatch();
    const root = rootRef.current;
    const ownerDocument = _nullishCoalesce(_optionalChain([root, 'optionalAccess', _24 => _24.ownerDocument]), () => ( document));
    const view = _nullishCoalesce(ownerDocument.defaultView, () => ( window));
    const vetoed = (reason, event) => _optionalChain([optionsRef, 'access', _25 => _25.current, 'access', _26 => _26.shouldDismiss, 'optionalCall', _27 => _27(reason, event)]) === false;
    const containsTarget = (target) => _optionalChain([rootRef, 'access', _28 => _28.current, 'optionalAccess', _29 => _29.contains, 'call', _30 => _30(target)]) || optionsRef.current.insideRefs.some((insideRef) => _optionalChain([insideRef, 'optionalAccess', _31 => _31.current, 'optionalAccess', _32 => _32.contains, 'call', _33 => _33(target)]));
    const onPointerDown = (event) => {
      if (!isTopmost()) return;
      if (!optionsRef.current.outsidePress || containsTarget(event.target)) return;
      if (vetoed("outside-press", event)) return;
      _optionalChain([optionsRef, 'access', _34 => _34.current, 'access', _35 => _35.onDismiss, 'optionalCall', _36 => _36("outside-press")]);
    };
    const onKeyDown = (event) => {
      ...
      const trigger = _optionalChain([optionsRef, 'access', _37 => _37.current, 'access', _38 => _38.getTrigger, 'optionalCall', _39 => _39()]);
      const activeElement = ownerDocument.activeElement;
      const ownsFocus = !!activeElement && containsTarget(activeElement);
      if (ownsFocus) latchDismissedTrigger();
      _optionalChain([optionsRef, 'access', _40 => _40.current, 'access', _41 => _41.onDismiss, 'optionalCall', _42 => _42("escape")]);
      if (!ownsFocus || activeElement === trigger) return;
      view.requestAnimationFrame(() => {
        if (_optionalChain([trigger, 'optionalAccess', _43 => _43.isConnected]) && typeof trigger.focus === "function") {
...
function useFloatingPosition({
  ...
  useSafeLayoutEffect2(() => {
    ...
    const view = _nullishCoalesce(_optionalChain([ownerDocument, 'optionalAccess', _44 => _44.defaultView]), () => ( window));
    ...
    _optionalChain([observer, 'optionalAccess', _45 => _45.observe, 'call', _46 => _46(anchor)]);
    _optionalChain([observer, 'optionalAccess', _47 => _47.observe, 'call', _48 => _48(panel)]);
    const boundaryElement = resolveCollisionBoundary(collisionBoundary, ownerDocument);
    if (boundaryElement && boundaryElement !== anchor && boundaryElement !== panel) _optionalChain([observer, 'optionalAccess', _49 => _49.observe, 'call', _50 => _50(boundaryElement)]);
    return () => {
      disposed = true;
      view.cancelAnimationFrame(frame);
      ...
      _optionalChain([observer, 'optionalAccess', _51 => _51.disconnect, 'call', _52 => _52()]);
    };
...
function findOverlayTrigger(root) {
  return _nullishCoalesce(_optionalChain([root, 'optionalAccess', _53 => _53.querySelector, 'call', _54 => _54('[data-anchored-overlay-trigger], button, [role="button"], a[href], input, select, textarea')]), () => ( null));
}
...
function partClassName(classNames, part, ...values) {
  return cx(...values, _optionalChain([classNames, 'optionalAccess', _55 => _55[part]]));
}
function partStyle(styles, part) {
  return _nullishCoalesce(_optionalChain([styles, 'optionalAccess', _56 => _56[part]]), () => ( void 0));
}
...
//# sourceMappingURL=chunk-CXNCDZQY.cjs.map