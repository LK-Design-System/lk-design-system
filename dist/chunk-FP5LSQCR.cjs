...
function Category({
  ...
  const groupRef = _react2.default.useRef(null);
  ...
  const tabStopValue = selectedItem && !selectedItem.disabled ? selectedItem.value : _optionalChain([normalized, 'access', _6 => _6.find, 'call', _7 => _7((item) => !item.disabled), 'optionalAccess', _8 => _8.value]);
  const pick = (item) => {
    if (item.disabled) return;
    if (!controlled) setInternal(item.value);
    _optionalChain([onChange, 'optionalCall', _9 => _9(item.value, item)]);
  };
...
//# sourceMappingURL=chunk-FP5LSQCR.cjs.map