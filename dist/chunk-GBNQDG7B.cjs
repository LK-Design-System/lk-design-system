...
function normalizeOption(option) {
  if (option && typeof option === "object") {
    return {
      ...option,
      label: _nullishCoalesce(option.label, () => ( String(option.value)))
    };
  }
...
function findNearestEnabled(options, index, direction = 1) {
  ...
  if (!_optionalChain([options, 'access', _ => _[bounded], 'optionalAccess', _2 => _2.disabled])) return bounded;
...
function WheelPicker({
  ...
  const opts = _react2.default.useMemo(() => options.map(normalizeOption), [options]);
  const controlled = value !== void 0;
  const firstEnabledIndex = findNearestEnabled(opts, 0, 1);
  const [internal, setInternal] = _react2.default.useState(() => {
    if (defaultValue != null) return defaultValue;
    return firstEnabledIndex >= 0 ? _optionalChain([opts, 'access', _3 => _3[firstEnabledIndex], 'optionalAccess', _4 => _4.value]) : void 0;
  });
  ...
  const scrollSettleTimer = _react2.default.useRef(null);
  const programmaticScroll = _react2.default.useRef(false);
  const typeahead = _react2.default.useRef({ query: "", at: 0 });
  const listId = _react2.default.useId();
  ...
  return /* @__PURE__ */ _jsxruntime.jsx.call(void 0,
    "div",
    {
      ...
      style: {
        ...
        boxShadow: _nullishCoalesce(focusShadow, () => ( "none")),
...
//# sourceMappingURL=chunk-GBNQDG7B.cjs.map