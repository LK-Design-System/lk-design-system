...
function insetCollisionRect(view, boundary, padding) {
  ...
  const source = _nullishCoalesce(_optionalChain([boundary, 'optionalAccess', _ => _.getBoundingClientRect, 'call', _2 => _2()]), () => ( viewport));
...
function useControllableOpen({ open, defaultOpen = false, onOpenChange }) {
  ...
  const setVisible = _react2.default.useCallback((nextValue) => {
    ...
    if (next !== visible) _optionalChain([onOpenChange, 'optionalCall', _3 => _3(next)]);
...
function useLightDismiss({
  ...
  _react2.default.useEffect(() => {
    ...
    releaseFocusLatch();
    const root = rootRef.current;
    const ownerDocument = _nullishCoalesce(_optionalChain([root, 'optionalAccess', _4 => _4.ownerDocument]), () => ( document));
    const view = _nullishCoalesce(ownerDocument.defaultView, () => ( window));
    const vetoed = (reason, event) => _optionalChain([optionsRef, 'access', _5 => _5.current, 'access', _6 => _6.shouldDismiss, 'optionalCall', _7 => _7(reason, event)]) === false;
    const containsTarget = (target) => _optionalChain([rootRef, 'access', _8 => _8.current, 'optionalAccess', _9 => _9.contains, 'call', _10 => _10(target)]) || optionsRef.current.insideRefs.some((insideRef) => _optionalChain([insideRef, 'optionalAccess', _11 => _11.current, 'optionalAccess', _12 => _12.contains, 'call', _13 => _13(target)]));
    const onPointerDown = (event) => {
      if (!isTopmost()) return;
      if (!optionsRef.current.outsidePress || containsTarget(event.target)) return;
      if (vetoed("outside-press", event)) return;
      _optionalChain([optionsRef, 'access', _14 => _14.current, 'access', _15 => _15.onDismiss, 'optionalCall', _16 => _16("outside-press")]);
    };
    const onKeyDown = (event) => {
      ...
      const trigger = _optionalChain([optionsRef, 'access', _17 => _17.current, 'access', _18 => _18.getTrigger, 'optionalCall', _19 => _19()]);
      const activeElement = ownerDocument.activeElement;
      const ownsFocus = !!activeElement && containsTarget(activeElement);
      if (ownsFocus) latchDismissedTrigger();
      _optionalChain([optionsRef, 'access', _20 => _20.current, 'access', _21 => _21.onDismiss, 'optionalCall', _22 => _22("escape")]);
      if (!ownsFocus || activeElement === trigger) return;
      view.requestAnimationFrame(() => {
        if (_optionalChain([trigger, 'optionalAccess', _23 => _23.isConnected]) && typeof trigger.focus === "function") {
...
function useFloatingPosition({
  ...
  useSafeLayoutEffect(() => {
    ...
    const view = _nullishCoalesce(_optionalChain([ownerDocument, 'optionalAccess', _24 => _24.defaultView]), () => ( window));
    ...
    _optionalChain([observer, 'optionalAccess', _25 => _25.observe, 'call', _26 => _26(anchor)]);
    _optionalChain([observer, 'optionalAccess', _27 => _27.observe, 'call', _28 => _28(panel)]);
    const boundaryElement = resolveCollisionBoundary(collisionBoundary, ownerDocument);
    if (boundaryElement && boundaryElement !== anchor && boundaryElement !== panel) _optionalChain([observer, 'optionalAccess', _29 => _29.observe, 'call', _30 => _30(boundaryElement)]);
    return () => {
      disposed = true;
      view.cancelAnimationFrame(frame);
      ...
      _optionalChain([observer, 'optionalAccess', _31 => _31.disconnect, 'call', _32 => _32()]);
    };
...
function findOverlayTrigger(root) {
  return _nullishCoalesce(_optionalChain([root, 'optionalAccess', _33 => _33.querySelector, 'call', _34 => _34('[data-anchored-overlay-trigger], button, [role="button"], a[href], input, select, textarea')]), () => ( null));
}
...
exports.useControllableOpen = useControllableOpen; exports.useLightDismiss = useLightDismiss; exports.useFloatingPosition = useFloatingPosition; exports.inlineFloatingStyle = inlineFloatingStyle; exports.appendAriaReference = appendAriaReference; exports.findOverlayTrigger = findOverlayTrigger;
//# sourceMappingURL=chunk-7ECMFW7H.cjs.map