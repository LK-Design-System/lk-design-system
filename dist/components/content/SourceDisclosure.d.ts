...
export interface SourceDisclosureProps extends Omit<React.HTMLAttributes<HTMLElement>, 'title'> {
  ...
  compact?: boolean;
  /** Collapse the compact source list behind a single "출처" icon+label toggle (no pill, matching the footer action controls) that opens it in an anchored Popover (dropdown), so the surrounding layout never shifts. The toggle is the disclosure's accessible name — no landmark heading is projected — and the floating panel (which lists each source as a borderless row) closes on outside-press or Escape. @default false */
  collapsible?: boolean;
  /** Start with the collapsible source popover open. Only applies when `collapsible` is set. @default false */
  defaultOpen?: boolean;
}
