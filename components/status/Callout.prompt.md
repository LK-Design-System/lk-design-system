**Callout** — 본문 흐름에 계속 남는 절차, 주의, 맥락 설명을 톤 아이콘과 낮은 톤 배경으로 묶는 외곽선 없는 정적 노트 블록입니다. 같은 tone의 `Banner`보다 한 단계 무거운 표면이며, 그 위계는 문장이 아니라 기하로 정의됩니다.
...
- **action** 은 안내와 직접 연결된 단일 저강조 다음 행동입니다. 기본 조합은 `TextButton tone="inherit" size="sm"` 또는 `Link tone="inherit"`이며, action이 없으면 기존 정적 Callout과 동일하게 렌더됩니다.
...
- action은 title과 children 뒤 content column에 놓고 별도 separator나 outlined container를 만들지 않습니다.
- 좁은 폭에서도 action은 본문 아래에서 자연스럽게 줄바꿈하며 가로 overflow를 만들지 않습니다.
- 접근성상 DOM과 키보드 순서는 제목 → 본문 → action을 유지합니다.
- 표면은 tone의 semantic color를 섞은 충분히 구분되는 tint만으로 경계를 만듭니다. 전체 외곽선, 그림자, 별도의 왼쪽 강조선이나 `outlined` appearance 축은 추가하지 않습니다.
