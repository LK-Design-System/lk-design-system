...
- [Primer Banner](https://primer.style/product/components/banner/)의 card 안 `flush` 배치처럼, 컨테이너 상태는 별도의 둥근 카드로 중첩하지 않고 부모 표면의 폭에 결합합니다. LDS에서는 이를 `variant="embedded"`로 명시합니다.
