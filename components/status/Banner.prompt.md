...
- **tone** — canonical `signal · positive · cautionary · negative`를 받으며 기본값은 `signal`입니다. 기존 `info · success · warning · error`도 별칭으로 계속 동작하지만 **동결된 어휘**라 새 코드에서는 쓰지 않습니다(`check:api-grammar`가 ledger에 없는 별칭 신규 도입을 차단합니다). 기본값이 한동안 `"info"`로 표기돼 있었는데, 별칭 정규화를 거쳐 `signal`과 같은 표면으로 렌더되던 같은 값이라 canonical 표기로 통일했습니다 — 렌더 결과는 바뀌지 않습니다. **variant** — `standalone`(기본) 또는 `embedded`. `standalone`은 상태 배경·아이콘·텍스트로 경계를 만들고 전체 외곽선을 두지 않습니다. 별도 `outlined` appearance 축도 제공하지 않습니다. **title / children** — 헤드라인 + 선택적 보충 설명. **action** — 상태를 직접 해결하거나 관련 상세로 이동하는 단일 노드. **onClose** — 닫기 버튼 표시. 떠 있는 일시 메시지에는 `Toast`를 쓰세요.
- `action`은 한 Banner에 하나만 두고, 기본 조합은 외곽선 없는 `TextButton tone="inherit" size="sm"` 또는 `Link tone="inherit"`입니다. 페이지 primary CTA와 경쟁하는 filled Button, 서로 다른 결정을 요구하는 복수 control, 메시지와 무관한 보조 도구는 넣지 않습니다. 복수 결정이 필요하면 해당 폼·패널의 `ActionArea`나 Dialog가 행동을 소유합니다. CTA 활성화가 Banner를 자동으로 닫지는 않으며 실행·loading·해소 후 제거는 제품 상태가 소유합니다.
...
- `variant="embedded"`는 WDS 원본 variant axis가 아니라 LDS composition extension입니다. 부모 패널의 header 바로 다음에 edge-to-edge로 배치하며, 부모가 외곽 border·radius·shadow를 소유합니다. Banner는 tone tint로 상태 영역을 구분하고 모든 자체 테두리와 radius를 제거합니다. `ValidationSummary`의 severity heading band는 여러 결과 구역을 나누는 별도 구조이므로 공유 `embeddedBandStyle`의 상·하단 separator를 계속 사용하며, 이번 Banner 결정의 영향을 받지 않습니다.
...
- [Atlassian Banner와 Section Message](https://atlassian.design/foundations/content/designing-messages/) 및 [MUI Alert의 기본 `standard` 변형](https://mui.com/material-ui/react-alert/#variants)처럼, 상태 알림은 톤 배경·아이콘·텍스트만으로 식별하고 전체 외곽선을 기본 시각 문법으로 사용하지 않습니다. LDS는 standalone과 embedded 모두 이 낮은 chrome 방향을 기본 계약으로 정하고 별도 `outlined` 축을 추가하지 않습니다.
- [Primer Banner](https://primer.style/product/components/banner/)의 card 안 `flush` 배치처럼, 컨테이너 상태는 별도의 둥근 카드로 중첩하지 않고 부모 표면의 폭에 결합합니다. LDS에서는 이를 `variant="embedded"`로 명시합니다. Primer의 상·하단 separator는 채택하지 않습니다. LDS 카드에서는 부모 외곽선과 Banner tint가 이미 경계를 제공해 선을 더하면 이중 chrome으로 읽히기 때문입니다.
...
- [Adobe Spectrum Alert Banner](https://spectrum.adobe.com/page/alert-banner/)와 [Carbon Actionable Notification](https://carbondesignsystem.com/components/notification/usage/#actionable)처럼 메시지를 직접 해결하는 contextual action은 하나로 제한합니다. LDS는 외곽선 버튼 대신 부모 tone foreground를 상속하는 저강조 텍스트 action을 기본 조합으로 정합니다.
