...
- **접힘** — 제품 셸의 정식 접기/펼치기 수단은 `collapsible`의 명시적 토글입니다. persistent 표면의 36px 토글은 브랜드 영역과 분리되어 패널의 논리적 끝 divider 안쪽 같은 위치를 유지하고, `사이드바 접기`/`사이드바 펼치기` 이름·툴팁·`aria-expanded`·`aria-controls`를 함께 제공합니다. 접히면 아이콘 레일(`collapsedWidth` 기본 64): 라벨→툴팁, 배지→도트, 헤딩→헤어라인, 브랜드→`headerCollapsed`. 같은 버튼 DOM을 유지하므로 전환 뒤에도 focus를 잃지 않으며 reduced-motion 환경에서는 폭과 토글 전환이 즉시 완료됩니다.
- **overlay** — `overlay` peek는 공간이 제한된 데스크톱에서만 쓰는 선택 기능이며 persistent 경계 토글을 상속하지 않습니다. 접힌 overlay는 기존 inline control을 유지하고 hover뿐 아니라 키보드 focus 진입에서도 펼쳐지며 포인터와 focus가 모두 이탈하면 다시 접힙니다. 바깥 클릭과 Esc도 레일로 복귀시키고, Esc로 자식 항목이 사라질 때 focus는 레일에 남는 부모 항목으로 복구됩니다.
...
- [Fluent Nav usage](https://fluent2.microsoft.design/components/web/react/core/nav/usage) — 목적지는 link, category는 펼침/접힘이며 category 자체는 이동하지 않습니다. 축소 가능 내비게이션도 상태를 설명하는 명시적인 제어를 제공하므로 LDS는 경계 토글을 정식 수단으로 두고 hover peek를 선택 기능으로 제한합니다.
