...
const TOP_BAR_MENU_POINTER_GRACE_MS = 160;
  ...
  useSafeLayoutEffect(() => {
    ...
    if (typeof panel.showPopover === 'function' && !isPopoverOpen(panel)) {
      try {
        ...
        // The visibility styles below remain a functional fallback when the
        // browser cannot promote this menu to the top layer.
      }
    }
    ...
    const update = () => {
      ...
      const top = placement === 'top'
        ? Math.max(viewportPadding, anchorRect.top - menuGap - Math.min(panelRect.height, availableHeight))
 ...
 */
export function TopBar({ brand, children, actions, navigationLabel = '주 탐색', navAlign = 'start', sticky = false, bordered = true, dark = false, height = 64, style, ...rest }) {
  ...
  return (
    <header
      ...
      {...rest}
    >
      <style>{TOP_BAR_STYLES}</style>
      {brand != null && <div data-top-bar-brand style={{ display: 'flex', alignItems: 'center', flexShrink: 0, minWidth: 0 }}>{brand}</div>}
      {children != null
        ? (
          <TopBarToneContext.Provider value={tone}>
            {/* Single-row global navigation preserves the fixed header height.
                Touch, trackpad and keyboard-focus movement remain available. */}
            <nav aria-label={navigationLabel} data-top-bar-nav data-scrollbar-exception="single-row-global-navigation" style={{ display: 'flex', alignItems: 'center', alignSelf: 'stretch', flex: '1 1 auto', minWidth: 0, overflowX: 'auto', overflowY: 'hidden', scrollbarWidth: 'none', WebkitOverflowScrolling: 'touch', justifyContent: navAlign === 'center' ? 'safe center' : 'flex-start' }}>
        ...
        : <div style={{ flex: 1 }} />}
      {actions != null && (
        <div data-top-bar-actions data-scrollbar-exception="single-row-global-actions" style={{ minWidth: 0, maxWidth: '100%', flex: '0 1 auto', overflowX: 'auto', overflowY: 'hidden', scrollbarWidth: 'none', WebkitOverflowScrolling: 'touch' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2-5)', width: 'max-content', minWidth: '100%' }}>{actions}</div>
        </div>
      )}
    </header>
  );
...
export function TopBarNavItem({
  ...
  const resolvedMenuTriggerLabel = menuTriggerLabel
  ...
  return (
    ...
    >
      <PrimaryComp
        ...
        id={!href && hasMenu ? triggerId : undefined}
        data-top-bar-primary
        href={href}
        type={href ? undefined : 'button'}
        aria-current={active ? 'page' : undefined}
        aria-expanded={!href && hasMenu ? open : undefined}
        aria-controls={!href && hasMenu ? menuId : undefined}
      ...
      {menuItems?.length ? (
        <ul
          ...
          style={{
            ...
            overflowY: menuPosition.maxHeight != null ? 'auto' : undefined,
            ...
            transform: open
              ? 'translateY(0)'
              : `translateY(${menuPosition.placement === 'top' ? '-4px' : '4px'})`,
            ...
            return (
              <li key={`${item.href ?? ''}:${index}`} style={{ display: 'block' }}>
                <ItemComp
                  ...
                  aria-current={item.current === true ? 'page' : item.current || undefined}
                  onClick={(event) => {
                    dismissMenu();
                    item.onClick?.(event);
                  }}
