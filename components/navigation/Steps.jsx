        ...
        return (
          <li key={i} aria-current={active ? 'step' : undefined} style={{ display: 'flex', alignItems: 'flex-start', flex: last ? '0 0 auto' : '1 1 auto' }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, flexShrink: 0 }}>
              ...
              <span style={{ fontFamily: 'var(--font-sans)', fontSize: 'var(--label2-size)', fontWeight: active ? 'var(--fw-bold)' : 'var(--fw-medium)', letterSpacing: 0, color: active ? 'var(--color-semantic-label-normal)' : 'var(--color-semantic-label-alternative)', whiteSpace: 'nowrap' }}>
                {label}
                <span style={srOnly}>{done ? ' 완료' : active ? ' 현재 단계' : ' 예정'}</span>
              </span>
            </div>
            {!last && <span aria-hidden="true" style={{ flex: 1, height: 2, marginTop: 15, background: i < current ? 'var(--color-semantic-primary-normal)' : 'var(--color-semantic-line-solid-normal)', minWidth: 24 }} />}
          </li>
        );
      })}
    </ol>
