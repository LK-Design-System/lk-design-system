...
- 서피스는 `--color-semantic-inverse-background`, 텍스트는 고정 화이트 알파(0.62 / 0.45 / 0.38)입니다. 루트가 `data-theme="light"`라 두 모드 모두 `#1B1C1E` 어두운 띠로 보입니다. 다크에서 인버스 서피스가 흰색으로 뒤집히면 고정 화이트 알파 글자가 사라지기 때문입니다(TOKEN_GOVERNANCE "Inverse surfaces").
