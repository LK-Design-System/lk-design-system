    ...
    <div style={{ fontFamily: 'var(--font-sans)', ...style }} {...rest}>
      ...
      {footer !== null && (
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 24 }}>
          ...
          <button type="button" onClick={() => go(cur + 1)} disabled={cur === steps.length - 1} style={{ height: 44, padding: '0 20px', border: 'none', borderRadius: 'var(--radius-md)', background: 'var(--lk-accent-ink)', color: 'var(--text-on-signal)', cursor: cur === steps.length - 1 ? 'not-allowed' : 'pointer', opacity: cur === steps.length - 1 ? 0.5 : 1, fontFamily: 'var(--font-sans)', fontSize: 15, fontWeight: 'var(--fw-bold)' }}>다음</button>
        </div>
      )}
    </div>
  );
