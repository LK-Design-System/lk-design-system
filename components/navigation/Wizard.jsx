    ...
    <div style={{ fontFamily: 'var(--font-sans)', ...style }} {...rest}>
          ...
          return (
            <React.Fragment key={i}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                <span style={{ width: 32, height: 32, borderRadius: '50%', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: done ? 'var(--color-semantic-primary-normal)' : 'var(--color-semantic-background-elevated-normal)', border: `2px solid ${done || active ? 'var(--color-semantic-primary-normal)' : 'var(--color-semantic-line-solid-normal)'}`, color: done ? 'var(--color-semantic-static-white)' : active ? 'var(--color-semantic-primary-normal)' : 'var(--color-semantic-label-assistive)', fontSize: 14, fontWeight: 'var(--fw-bold)' }}>
      ...
      {footer !== null && (
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 24 }}>
          <button type="button" onClick={() => go(cur - 1)} disabled={cur === 0} style={{ height: 44, padding: '0 18px', border: '1px solid var(--color-semantic-line-solid-normal)', borderRadius: 'var(--radius-md)', background: 'var(--color-semantic-background-elevated-normal)', color: 'var(--color-semantic-label-normal)', cursor: cur === 0 ? 'not-allowed' : 'pointer', opacity: cur === 0 ? 0.5 : 1, fontFamily: 'var(--font-sans)', fontSize: 15, fontWeight: 'var(--fw-bold)' }}>이전</button>
