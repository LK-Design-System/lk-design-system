...
- **autoPlay / interval** — 자동 회전은 opt-in입니다. 켜면 일시정지 컨트롤이 첫 번째 포커스 대상으로 렌더되고(WCAG 2.2.2), 포인터가 올라가거나 내부로 포커스가 들어오면 회전이 멈추며, 이전·다음·점을 누르면 회전이 완전히 멈춥니다(APG). 회전을 멈추는 수단 없이 움직이는 콘텐츠를 5초 넘게 두지 마세요.
