...
Flex sizing 근거는 [CSS Flexible Box Layout Module Level 1](https://www.w3.org/TR/css-flexbox-1/#flexibility)의 content-based flex basis와 automatic minimum size, [MDN의 flex item wrapping 안내](https://developer.mozilla.org/en-US/docs/Web/CSS/Guides/Flexible_box_layout/Wrapping_items)의 “전체 item 폭이 컨테이너보다 클 때 wrap” 원칙입니다. LDS는 이 원칙을 `width: max-content`, `max-width: 100%`, `min-width: 0` 조합으로 적용해 intrinsic 폭 보존과 협소 폭 축소를 분리합니다. 필터 host와 wide control host는 `flex-shrink: 0`이라 검색과 같은 줄에 intrinsic 폭으로 들어가지 않으면 host 단위로 다음 줄로 내려가며(검색은 먼저 `min-width` 200px까지 줄어듭니다), 필터가 host 안에서 흩어져 두 줄로 쌓이는 중간 폭 상태를 만들지 않습니다. host가 한 줄 전체보다 넓은 극단 폭에서만 `max-width: 100%` 안에서 내부 줄바꿈이 일어나고, 그때 세로 간격은 `--space-2`(8px)로 32px control이 붙어 보이지 않게 합니다.
