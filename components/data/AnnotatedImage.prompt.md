...
- anatomy는 `figure` → media frame → overlay 표시 `ToggleIcon` → 시각 overlay → 직접 자식인 `figcaption`과 순서가 같은 텍스트 요약입니다.
...
- 영역 label은 box 내부에 두어 frame 밖으로 잘리지 않게 하고, point label은 좌표가 가장자리에 가까우면 반대 방향으로 배치합니다. 좁은 폭에서는 label보다 번호와 텍스트 summary를 우선합니다.
