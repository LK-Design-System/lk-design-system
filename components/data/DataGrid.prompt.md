...
- loading·error·empty는 같은 상태 행을 사용하며, 자동 높이에서는 compact한 최소 본문 높이를 유지합니다. 제품이 `height` 또는 stretch layout으로 높이를 할당하면 표 header를 제외한 가용 본문 영역의 가로·세로 중앙에 상태 메시지와 action을 배치합니다.
- 이 규칙은 [EmptyState의 컨테이너 배치 계약](../status/EmptyState.prompt.md#컨테이너-안의-배치)을 따릅니다. 메뉴나 자동 높이 목록에 고정 높이를 만드는 규칙으로 확장하지 않습니다.
