    ...
    <div style={{ display: 'flex', alignItems: 'flex-end', gap, height, fontFamily: 'var(--font-sans)', ...style }} {...rest}>
      {data.map((d, i) => (
        <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, height: '100%', justifyContent: 'flex-end' }}>
          {showValue && <span style={{ fontSize: 12, fontWeight: 'var(--fw-bold)', color: 'var(--color-semantic-label-neutral)', fontVariantNumeric: 'tabular-nums' }}>{d.value}</span>}
          ...
          <span style={{ fontSize: 12, color: 'var(--color-semantic-label-alternative)', whiteSpace: 'nowrap' }}>{d.label}</span>
        </div>
      ))}
    </div>
  );
