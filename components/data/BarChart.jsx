 ...
 */
export function BarChart({ data = [], height = 160, gap = 12, showValue = true, color = 'var(--color-semantic-primary-normal)', style, ...rest }) {
  const max = Math.max(...data.map((d) => d.value), 1);
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap, height, fontFamily: 'var(--font-sans)', ...style }} {...rest}>
      {data.map((d, i) => (
        <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, height: '100%', justifyContent: 'flex-end' }}>
          {showValue && <span style={{ fontSize: 'var(--caption1-size)', fontWeight: 'var(--fw-bold)', color: 'var(--color-semantic-label-neutral)', fontVariantNumeric: 'tabular-nums' }}>{d.value}</span>}
          ...
          <span style={{ fontSize: 'var(--caption1-size)', color: 'var(--color-semantic-label-alternative)', whiteSpace: 'nowrap' }}>{d.label}</span>
        </div>
      ))}
    </div>
  );
