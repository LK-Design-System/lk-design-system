...
export function SplitButton({ children, onClick, items = [], variant = 'primary', size = 'md', style, ...rest }) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative', display: 'inline-flex', ...style }} {...rest}>
      ...
      <button type="button" aria-label="more actions" onClick={() => setOpen((o) => !o)} style={{ height: h, width: 42, border: 'none', borderLeft: '1px solid var(--color-semantic-inverse-line-strong)', borderTopRightRadius: 'var(--radius-md)', borderBottomRightRadius: 'var(--radius-md)', background: pal.bg, color: pal.fg, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      {open && (
        <div role="menu" style={{ position: 'absolute', top: 'calc(100% + 8px)', right: 0, zIndex: 40, minWidth: 184, background: 'var(--color-semantic-background-elevated-normal)', border: '1px solid var(--color-semantic-line-solid-normal)', borderRadius: 'var(--component-menu-radius)', boxShadow: 'var(--shadow-md)', padding: 'var(--component-menu-padding-y) var(--component-menu-padding-x)' }}>
          {items.map((it, i) => (
