...
```jsx
<Button variant="primary" arrow>도입 문의</Button>
<Button variant="signal" size="lg">제품 보기</Button>
<Button variant="ghost">자세히 보기</Button>
<Button variant="on-dark" arrow>Learn more</Button>
...
- **size** — `sm` 44px · `md` 52px · `lg` 52px+. **arrow**는 끝에 →를 더해 호버 시 살짝 움직입니다. **full**은 컨테이너 폭까지 늘립니다. **as="a"**는 링크 CTA로 렌더합니다.
- 차분한 모션: 호버 시 미세한 리프트 + 그림자, 프레스 시 약간 축소. 바운스 없음.
