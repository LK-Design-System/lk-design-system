  ...
  return (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, height: h, width: '100%', padding: '0 12px 0 14px', boxSizing: 'border-box', background: 'var(--color-semantic-background-elevated-normal)', border: `1px solid ${focus ? 'var(--color-semantic-primary-normal)' : 'var(--color-semantic-line-solid-normal)'}`, borderRadius: 'var(--radius-input)', boxShadow: focus ? '0 0 0 4px var(--color-semantic-focus-ring)' : 'none', opacity: disabled ? 0.5 : 1, transition: 'border-color var(--dur-fast) var(--ease-out), box-shadow var(--dur-fast) var(--ease-out)', ...style }}>
      <input value={val} disabled={disabled} placeholder={placeholder} aria-label={ariaLabel ?? (typeof placeholder === 'string' ? placeholder : '비밀번호')} type={show ? 'text' : 'password'} onChange={(e) => set(e.target.value)} onFocus={() => setFocus(true)} onBlur={() => setFocus(false)}
        style={{ flex: 1, minWidth: 0, border: 'none', outline: 'none', background: 'transparent', fontFamily: 'var(--font-sans)', fontSize: 15, color: 'var(--color-semantic-label-normal)' }} {...rest} />
