...
export function Combobox({ options = [], value, defaultValue = [], onChange, placeholder = '선택', size = 'md', style, ...rest }) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative', ...style }} {...rest}>
      <button type="button" onClick={() => setOpen((o) => !o)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, width: '100%', minHeight: h, padding: '6px 12px', boxSizing: 'border-box', background: 'var(--bw-white)', border: `1px solid ${open ? 'var(--lk-accent-ink)' : 'var(--bw-border)'}`, borderRadius: 'var(--radius-input)', cursor: 'pointer', fontFamily: 'var(--font-sans)', fontSize: 15, color: sel.length ? 'var(--label-normal)' : 'var(--label-assistive)' }}>
        <span style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
          {sel.length ? sel.map((v) => <span key={v} style={{ display: 'inline-flex', height: 24, alignItems: 'center', padding: '0 9px', background: 'var(--lk-accent-tint-2)', color: 'var(--lk-accent-ink)', borderRadius: 'var(--radius-pill)', fontSize: 13, fontWeight: 'var(--fw-semibold)' }}>{labelFor(v)}</span>) : placeholder}
        </span>
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--label-alternative)" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}><path d="m6 9 6 6 6-6" /></svg>
      </button>
      {open && (
