 ...
 * Compatibility multi-select for short, non-searchable option lists. Prefer
 * SearchableMultiSelect when search, async data, descriptions, or disabled
 * options are part of the reusable contract. Value is a string[].
 */
    ...
    <div ref={ref} style={{ position: 'relative', ...style }} {...rest}>
      <button type="button" onClick={() => setOpen((o) => !o)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, width: '100%', minHeight: h, padding: '6px 12px', boxSizing: 'border-box', background: 'var(--color-semantic-background-elevated-normal)', border: `1px solid ${open ? 'var(--color-semantic-primary-normal)' : 'var(--color-semantic-line-solid-normal)'}`, borderRadius: 'var(--radius-input)', cursor: 'pointer', fontFamily: 'var(--font-sans)', fontSize: 15, color: sel.length ? 'var(--color-semantic-label-normal)' : 'var(--color-semantic-label-assistive)' }}>
        <span style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
          {sel.length ? sel.map((v) => <span key={v} style={{ display: 'inline-flex', height: 24, alignItems: 'center', padding: '0 9px', background: 'var(--color-semantic-primary-surface-strong)', color: 'var(--color-semantic-primary-normal)', borderRadius: 'var(--radius-pill)', fontSize: 13, fontWeight: 'var(--fw-semibold)' }}>{labelFor(v)}</span>) : placeholder}
        </span>
      ...
      {open && (
        <div role="listbox" style={{ position: 'absolute', top: 'calc(100% + 6px)', left: 0, right: 0, zIndex: 40, maxHeight: 260, overflowY: 'auto', background: 'var(--color-semantic-background-elevated-normal)', border: '1px solid var(--color-semantic-line-solid-normal)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-md)', padding: 6, display: 'flex', flexDirection: 'column', gap: 2 }}>
          {norm.map((o) => {
