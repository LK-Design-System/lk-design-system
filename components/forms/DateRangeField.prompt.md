...
- 종료일이 시작일보다 빠르면 group을 invalid로 표시하고 `role="alert"` 오류를 제공합니다. 제품은 서버 정책이나 timezone 검증을 `invalid/errorMessage`로 추가할 수 있습니다.
...
내부 `DatePicker`, `Calendar`, `FormField`, `FilterChip`의 입력 높이·focus ring·popup 동작을 재사용했습니다. [Carbon Date picker usage](https://carbondesignsystem.com/components/date-picker/usage/)는 range의 시작/종료 label, toolbar용 조밀한 크기, 가변 필드 폭을 명시하고, [WAI-ARIA Date Picker Dialog example](https://www.w3.org/WAI/ARIA/apg/patterns/dialog-modal/examples/datepicker-dialog/)은 이름·키보드·focus 복귀 계약을 제공합니다.
