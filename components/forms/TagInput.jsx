...
export function TagInput({ value, defaultValue = [], onChange, placeholder = '입력 후 Enter', disabled = false, style, ...rest }) {
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center', minHeight: 50, padding: '8px 10px', border: '1px solid var(--bw-border)', borderRadius: 'var(--radius-input)', background: 'var(--bw-white)', opacity: disabled ? 0.5 : 1, ...style }} {...rest}>
      {tags.map((t) => (
        <span key={t} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, height: 28, padding: '0 6px 0 11px', background: 'var(--lk-accent-tint-2)', color: 'var(--lk-accent-ink)', borderRadius: 'var(--radius-pill)', fontFamily: 'var(--font-sans)', fontSize: 13.5, fontWeight: 'var(--fw-semibold)' }}>
          {t}
          <button type="button" aria-label="remove" onClick={() => remove(t)} style={{ display: 'inline-flex', border: 'none', background: 'transparent', cursor: 'pointer', color: 'var(--lk-accent-ink)', padding: 2 }}>
