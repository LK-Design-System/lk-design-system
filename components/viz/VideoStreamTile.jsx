    ...
    >
      ...
      {status === 'loading' && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center',
          ...
          <span style={{ width: 30, height: 30, borderRadius: '50%', border: '3px solid rgba(255,255,255,0.22)', borderTopColor: 'var(--lk-accent)', animation: 'lk-stream-spin 0.8s linear infinite' }} />
        </div>
      )}
      {status === 'disconnected' && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'center', justifyContent: 'center',
          background: 'rgba(8,14,33,0.72)', color: 'rgba(255,255,255,0.55)', fontSize: 13, fontWeight: 600, whiteSpace: 'nowrap' }}>
