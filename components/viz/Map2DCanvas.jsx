  ...
  return (
    <div onPointerDown={down} onPointerMove={move} onPointerUp={up} onPointerCancel={up} onWheel={wheel}
      style={{ position: 'relative', overflow: 'hidden', width: '100%', height: '100%', minHeight: 200, borderRadius: 'var(--radius-lg)',
        border: '1px solid var(--color-semantic-line-normal-normal)', background: 'var(--color-semantic-background-normal-alternative)', cursor: 'grab', touchAction: 'none', fontFamily: 'var(--font-sans)',
      ...
      {controls && <span style={{ position: 'absolute', left: 10, bottom: 10, fontSize: 11, fontWeight: 'var(--fw-bold)', color: 'var(--color-semantic-label-alternative)', background: 'var(--color-semantic-background-elevated-normal)', border: '1px solid var(--color-semantic-line-normal-normal)', borderRadius: 'var(--radius-sm)', padding: '2px 7px', fontVariantNumeric: 'tabular-nums' }}>{Math.round(t.z * 100)}%</span>}
    </div>
  );
}
