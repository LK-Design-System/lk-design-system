...
- `value / min / max / unit`은 meter 범위와 값을 정의합니다. 기본 표시는 `String(value)`를 사용해 `1.4`를 `1`로 반올림하지 않습니다.
- `precision`은 소비자가 명시적으로 고정 소수 자릿수를 원할 때만 사용합니다. `formatter`는 text/number를 반환하며 그 결과가 기본 `aria-valuetext`에도 반영됩니다. 도메인 문구가 더 필요할 때 `valueText`로 명시합니다.
