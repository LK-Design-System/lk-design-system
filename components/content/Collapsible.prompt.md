...
- **density / align** — `default`는 독립된 본문 섹션에 사용합니다. `compact`는 이미지 캡션·메타데이터처럼 작은 텍스트 문맥에 조합하는 밀도이며, caption 토큰과 최소 24px 트리거 높이를 사용합니다. `align="start" | "end"`는 트리거와 패널을 같은 내용 너비 블록으로 정렬하며, 기본값 `stretch`는 둘 다 전체 너비를 채웁니다.
