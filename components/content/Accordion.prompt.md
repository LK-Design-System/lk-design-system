...
- **leading / description** — 트리거 안에 제목 앞 장식 노드와 제목 아래 보조 설명을 둡니다. 둘 다 **트리거 안**에 있어 행 전체가 계속 눌리지만, **버튼의 접근 이름은 `aria-labelledby`로 `title`에만 고정**되어 있어 장식 아이콘이나 설명이 이름에 섞이지 않습니다. 설명은 대신 `aria-describedby`로 연결되어 이름은 짧게, 설명은 설명으로 낭독됩니다. `title`에 아이콘을 직접 넣으면 그 아이콘이 heading·버튼 이름 안으로 들어가므로 그렇게 하지 마세요.
