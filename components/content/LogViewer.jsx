    ...
    <div style={{ display: 'grid', gap: 'var(--space-2)', width: '100%', maxWidth: '100%', fontFamily: 'var(--font-sans)', ...style }} {...rest}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 'var(--space-3)', flexWrap: 'wrap', color: 'var(--color-semantic-label-neutral)', fontSize: 'var(--caption1-size)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', flexWrap: 'wrap' }}>
            {streamStatus != null && (() => {
              ...
              return <StatusIndicator tone={cfg.tone} pulse={cfg.pulse} data-status={streamStatus}>{cfg.label}</StatusIndicator>;
            })()}
      ...
      {(filter || search || tools) && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          {filter && (
            <div style={{ display: 'flex', gap: 'var(--space-1-5)', flexWrap: 'wrap', flex: '1 1 220px' }}>
              {ORDER.map((lvl) => {
          ...
          {search && (
            <label style={{ flex: '1 1 180px', maxWidth: 260, minWidth: 160, height: 28, display: 'inline-flex', alignItems: 'center', gap: 'var(--space-2)', padding: '0 9px', boxSizing: 'border-box', border: '1px solid var(--color-semantic-line-normal-normal)', borderRadius: 'var(--radius-sm)', background: 'var(--color-semantic-background-elevated-normal)', color: 'var(--color-semantic-label-assistive)' }}>
      ...
      <div ref={boxRef} className="lk-scroll-surface" data-scrollbar="auto" data-scroll-gutter="stable" role="log" aria-live="off" aria-label={ariaLabel} tabIndex={0} onScroll={updateScrollState} style={{ height, overflow: 'auto', scrollbarGutter: 'stable', padding: metrics.panelPadding, borderRadius: 'var(--radius-md)', background: 'var(--color-semantic-inverse-background)', border: '1px solid var(--color-semantic-inverse-line-normal)', fontFamily: 'var(--font-mono)', fontSize: metrics.fontSize, lineHeight: metrics.lineHeight }}>
        ...
        {visibleLines.map((line, visibleIndex) => {
          ...
          return (
            <div key={index} style={{ display: 'grid', gridTemplateColumns, columnGap: 'var(--space-2-5)', alignItems: 'baseline', minHeight: metrics.rowMinHeight, height: virtualActive ? rowHeight : undefined, overflow: virtualActive ? 'hidden' : undefined, whiteSpace: virtualActive || !wrap ? 'nowrap' : 'pre-wrap', wordBreak: virtualActive || !wrap ? 'normal' : 'break-word' }}>
