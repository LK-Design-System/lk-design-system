    ...
    <div role="presentation" onClick={(e) => { if (e.target === e.currentTarget && onClose) onClose(); }} style={{ position: 'fixed', inset: 0, zIndex: 110, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '12vh', background: 'var(--scrim-dark)', backdropFilter: 'blur(2px)' }}>
      <div role="dialog" aria-modal="true" style={{ width: '100%', maxWidth: 560, background: 'var(--bw-white)', borderRadius: 'var(--radius-2xl)', boxShadow: 'var(--shadow-xl)', overflow: 'hidden', fontFamily: 'var(--font-sans)', ...style }} {...rest}>
        ...
        <div style={{ maxHeight: 340, overflowY: 'auto', padding: 8 }}>
          {filtered.length === 0
            ? <div style={{ padding: 28, textAlign: 'center', color: 'var(--label-alternative)', fontSize: 14 }}>결과 없음</div>
            : filtered.map((c, i) => (
              <button key={i} type="button" onClick={() => { if (onClose) onClose(); c.onSelect && c.onSelect(); }} onMouseEnter={(e) => { e.currentTarget.style.background = 'var(--fill-normal)'; }} onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent'; }}
                ...
                {c.icon && <span style={{ color: 'var(--lk-accent-ink)', display: 'inline-flex' }}>{c.icon}</span>}
                <span style={{ flex: 1 }}>{c.label}</span>
                {c.shortcut && <span style={{ fontSize: 12, color: 'var(--label-assistive)', fontWeight: 'var(--fw-semibold)' }}>{c.shortcut}</span>}
              </button>
            ))}
        </div>
      </div>
