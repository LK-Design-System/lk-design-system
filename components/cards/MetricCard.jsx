  ...
  return (
    <div style={{ background: 'var(--component-card-bg)', border: 'var(--component-card-border)', borderRadius: 'var(--component-card-radius)', padding: '22px 24px', boxShadow: 'var(--shadow-xs)', fontFamily: 'var(--font-sans)', ...style }} {...rest}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 14 }}>
        <span style={{ fontSize: 12, fontWeight: 'var(--fw-bold)', letterSpacing: '1.4px', textTransform: 'uppercase', color: 'var(--color-semantic-label-alternative)' }}>{label}</span>
        {icon && <span style={{ color: 'var(--color-semantic-primary-normal)', display: 'inline-flex' }}>{icon}</span>}
      </div>
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 10, flexWrap: 'wrap' }}>
        <span style={{ fontSize: 34, fontWeight: 'var(--fw-extra)', letterSpacing: 0, color: 'var(--color-semantic-label-normal)', lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>{value}</span>
        {delta != null && (
      ...
      {caption != null && <div style={{ marginTop: 8, fontSize: 13, color: 'var(--color-semantic-label-alternative)' }}>{caption}</div>}
    </div>
  );
}
