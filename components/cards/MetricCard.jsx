...
export function MetricCard({ label, value, delta, deltaTone = 'auto', caption, icon, style, ...rest }) {
  ...
  const up = tone === 'up';
  const dc = up ? 'var(--color-semantic-status-positive-text)' : tone === 'down' ? 'var(--color-semantic-status-negative-text)' : 'var(--color-semantic-label-alternative)';
  ...
  return (
    <div style={{ background: 'var(--component-card-bg)', border: 'var(--component-card-border)', borderRadius: 'var(--component-card-radius)', padding: '22px 24px', boxShadow: 'var(--shadow-xs)', fontFamily: 'var(--font-sans)', ...style }} {...rest}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 14 }}>
        <span style={{ fontSize: 'var(--caption1-size)', fontWeight: 'var(--fw-bold)', letterSpacing: '1.4px', textTransform: 'uppercase', color: 'var(--color-semantic-label-alternative)' }}>{label}</span>
        {icon && <span style={{ color: 'var(--color-semantic-primary-normal)', display: 'inline-flex' }}>{icon}</span>}
      </div>
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 10, flexWrap: 'wrap' }}>
        <span style={{ fontSize: 'var(--title1-size)', fontWeight: 'var(--fw-extra)', letterSpacing: 0, color: 'var(--color-semantic-label-normal)', lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>{value}</span>
        {delta != null && (
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3, fontSize: 'var(--label2-size)', fontWeight: 'var(--fw-bold)', color: dc }}>
      ...
      {caption != null && <div style={{ marginTop: 8, fontSize: 'var(--label2-size)', color: 'var(--color-semantic-label-alternative)' }}>{caption}</div>}
    </div>
  );
}
