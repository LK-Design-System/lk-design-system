...
- **작성자 헤더는 `ListCell`을 재사용** — leading 아바타(`Avatar`) + title 이름 + description `meta` + trailing(팔로우 `Button`·오버플로 `DropdownMenu`). 새 헤더 컴포넌트를 만들지 않습니다. `headingLevel`을 주면 이름이 heading이 되어 피드 문서 아웃라인을 만들고, `author.href`면 이름이 프로필 링크가 됩니다.
...
- 여러 장은 `role="feed"` 컨테이너나 `Grid`로 배치하고, 각 게시물의 로딩·정렬은 제품이 소유합니다.
