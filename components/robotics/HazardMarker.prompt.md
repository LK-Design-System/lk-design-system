...
- `hazard`는 `kind="stairs" | "ramp" | "dropoff" | "obstacle"`, `severity="caution" | "danger"`로 구분되는 직렬화 가능한 데이터입니다. `id`·`label`·`mapId`·`position`은 필수이며, renderer 핸들을 저장하지 않습니다.
- `obstacle`(충돌 위험물)은 **정적/준정적으로 등록된 충돌 지점**입니다 — 기둥, 돌출 설비, 낮은 보/배관 같은 높이 제한물, 상시 적치 구역. 사람·지게차·떨어진 적재물처럼 로봇 센서가 실시간으로 잡는 **동적 장애물은 제품의 live perception 레이어 소관**이며 이 정적 annotation으로 표현하지 않습니다. 높이 제한을 별도 kind로 나눌 필요가 생기기 전까지는 제품이 obstacle로 분류합니다.
...
- 핀 배지 위 knockout 글리프가 위험물 종류를 나타냅니다. `stairs`는 Material Symbols `stairs_2`(box 없는 계단)를 원본 그대로, `ramp`는 `_FacilityGlyph`의 LDS 경사로 실루엣을 그대로 재사용하고, `dropoff`는 Material Symbols에 낙하/단차 글리프가 없어 LDS가 그린 한 단짜리 edge 프로파일 + 낙하 화살표입니다 — 여러 단의 계단 지그재그와 badge 크기에서 구분되도록 의도적으로 한 단만 씁니다. `obstacle`은 물리적 장애물의 현장 표준 상징인 LDS 안전 콘 실루엣입니다 — Material Symbols의 근접 후보(fence·dangerous)는 badge 크기에서 격자 뭉개짐/금지 X로 잘못 읽혀 채택하지 않았습니다(`docs/references/ATTRIBUTIONS.md`).
...
- 현재 `kind`는 `stairs`·`ramp`·`dropoff`·`obstacle` 넷입니다. 후속 종류(예: 높이 제한 전용 clearance)가 필요해지면 같은 severity·핀 배지 체계에 knockout 글리프와 함께 편입합니다.
