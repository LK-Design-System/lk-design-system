...
- Pointer click and non-repeating Enter/Space call `onActivate(waypoint.id, event)`. `event.repeat` and `disabled` block activation; disabled preserves context but leaves the Tab order. Disabled opacity is `0.45`, while stale data uses `0.76`, matching the other Robotics Navigation renderers. `unavailable`, `invalid`, and `stale` remain selectable so operators can inspect the reason.
