...
- `availability`와 `conflict`는 runtime render prop이며 서로 독립입니다. `availability`는 `available | closed | unknown`이고, `closed + conflict`와 `available + conflict`를 모두 표현합니다. 통행 가능한 lane은 정적 인프라이므로 `--viewer-muted` 톤으로 조용하게 그립니다 — accent·상태 톤은 살아있는 것(현재 경로 구간·활성 궤적·로봇·선택 강조)의 몫이라, 한 지도에서 정적 topology와 동적 상태의 주부 위계가 유지됩니다. 상태 badge·엔드포인트·카운트 칩은 공유 `NAV_MARKER_SHADOW` 캐스트 그림자로 지면에 접지합니다.
...
- `onActivate`가 있을 때 pointer와 non-repeating `Enter`/`Space`가 같은 `(id, event)` callback을 호출합니다. `event.repeat`와 `disabled`는 callback을 막고, disabled interactive lane은 `aria-disabled`와 `tabIndex=-1`을 사용합니다. Disabled opacity는 `0.45`, stale opacity는 `0.76`으로 Facility/Waypoint 계열의 상태 계층과 맞춥니다. 이 상태 opacity·label halo·상태 badge·hit target 값은 공통 내부 `_navigationVocabulary` 모듈에서 가져오므로 sibling renderer와의 일치는 수기 약속이 아니라 구조적으로 보장됩니다(가용성·미확인 등 lane path dash는 component 고유 encoding이라 로컬로 유지).
...
- 색만으로 상태를 전달하지 않습니다. 폐쇄/미확인은 dash pattern과 registry close/question geometry, conflict·invalid는 별도 pattern과 registry exclamation geometry, stale은 adapted clock-hand geometry로 함께 표시합니다. 모든 상태 표식은 font fallback 없이 공통 `NavigationStateGlyph`의 최소 10px SVG geometry를 사용하고 `data-navigation-state-glyph`/source를 남깁니다. 상태 hue는 path와 badge 외곽선에 남기고 내부 geometry는 appearance-aware `--viewer-foreground`를 사용해 light/dark viewer surface에서 필요한 대비를 보존합니다. Direction은 공용 내부 모듈 `_navigationVectorGlyph`의 `NAV_DIRECTION_CHEVRON`을 최장 직선 구간의 중점에 한 번 표시합니다 — 선을 짧게 끊고 그 자리에서 선 자체가 같은 색·같은 두께의 열린 V로 꺾이므로, 별도 배지·부풀림 없이 선 그 자체가 화살표로 읽히고 코너에 걸치지 않습니다. 절개 창은 SVG 마스크로 lane 자신의 casing·path·conflict stroke에만 적용합니다 — 같은 회랑을 지나는 다른 오버레이(경로·궤적)의 선은 끊기지 않으며, surface 덧칠 지우개는 금지입니다. 셰브론 자리는 `NavigationAnnotationLayer` 장애물로 등록되어 조정 라벨이 덮지 않습니다. 이는 정적인 entry→exit 방향이고, Route·Trajectory의 elapsed 선 끝에 붙는 채움 화살촉 현재 진행 표식(`NAV_PROGRESS_TRIANGLE`)과 모양 자체가 다릅니다. 차선 종점 방향 화살표는 endpoint orientation이라는 별도 역할이라 lane 로컬 geometry로 둡니다.
- 자산 suitability inventory에서 `assets/icons/question.svg`, `exclamation.svg`, `close.svg`, `clock.svg`와 생성된 `Icon` registry를 먼저 확인했습니다. question/exclamation/close는 registry path를 직접 재사용하고, stale은 badge circle을 clock perimeter로 삼아 `clock.svg` hands만 축약합니다. 방향은 문자 `>`·폰트 화살표·떠 있는 독립 glyph·배지형 캡슐을 사용하지 않고, 선을 절개한 창 안에서 path tone·path 두께 그대로의 열린 V로 꺾어 어떤 상태색 위에서도 선 그 자체가 방향으로 읽히게 합니다.
...
| `WaypointMarker` | inverse viewport scale, 24px target, selected/focused/disabled/invalid/stale vocabulary | point 대신 방향 polyline이므로 path halo와 한 번만 배치된 선 절개 방향 셰브론을 사용 |
...
- [Google Maps polyline symbols](https://developers.google.com/maps/documentation/javascript/symbols)와 대중교통 노선도의 in-line 화살표는 방향 표식을 선 자체에 결합해 어떤 지도 배경에서도 선의 일부로 읽히게 합니다. LDS Lane은 같은 계열의 선 절개 셰브론 — 선을 끊고 선 자체가 여는 V로 꺾이는 문법 — 을 전체 선에서 한 번, 최장 직선 구간의 중점에 배치해 entry→exit 방향을 표시하고, 반복 표식과 떠 있는 독립 glyph는 두지 않습니다.
- [W3C SVG Markers](https://www.w3.org/TR/svg-markers/)의 path-relative marker와 automatic orientation은 Route·Trajectory 진행 화살촉의 구현 근거입니다. Lane의 정적 방향은 marker가 아니라 선 위에 직접 그리는 회전 절개 셰브론이며, 진행 표식과 기하·`data-*`·배치를 전부 분리해 한 지도에서 두 의미가 혼동되지 않습니다.
- [MapLibre Style Spec — line and symbol layers](https://maplibre.org/maplibre-style-spec/layers/)는 line geometry와 symbol placement/collision을 별도 layer 계약으로 두고, `symbol-sort-key` 및 allow-overlap 설정으로 placement 우선순위를 정합니다. LDS fragment는 모든 label을 강제로 표시하거나 충돌 순위를 추론하지 않으며, owning renderer가 zoom·density·selection에 따라 label priority와 suppression을 결정합니다. 그 renderer 조각의 reference 구현은 `components/robotics/NavigationAnnotationLayer.jsx`이며, provider 없이 단독 렌더된 Lane은 오늘과 동일하게 동작합니다.
...
- LK Web Viz `a984def117c05acd213f494cbb8a42e990595505`: pinned `MapEditScreen`·`TaskRunScreen`·`Map2DViewer`의 point/line authoring과 runtime layer 분리를 기준으로 graph lane은 `supported by composition`입니다. 제품의 `forbidden` line은 방향성 graph edge가 아니므로 Lane으로 매핑하지 않고 `gap`으로 유지합니다.
- LK Control Full Daedeok `93802fc2aa5d29f930380ae58d51dcb68322b5e7`: pinned `InteractiveMap`에서 observed path와 robot pose 분리는 확인했지만 navigation-graph lane source mapping은 닫히지 않아 `unverified`입니다.
- LK Context Hub `de124084b7e50049350a46f92c4ea4476269c58c`: pinned frontend에 map·floor·robot navigation 진입점이 없어 `not applicable`입니다. 상세 source blob과 제품별 seam은 `docs/references/product-frontends/COVERAGE_AUDIT.json`과 `docs/PRODUCT_FRONTEND_COVERAGE.md`가 소유합니다.

## Intentional exclusions

- map projection, pixel/world transform, pan/zoom, layer ordering, overlap resolution
- waypoint renderer, planned-route progress, dense path interpolation, robot pose/footprint(`RobotMarker` 소관)
