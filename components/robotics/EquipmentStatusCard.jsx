...
export function EquipmentStatusCard({ icon, title, ringLabel, ringCaption, tone = 'neutral', direction, connection, chips, style, ...rest }) {
  ...
  return (
    ...
    >
      {icon != null && (
        <span style={{ width: 38, height: 38, borderRadius: 'var(--radius-md)', flexShrink: 0, background: 'var(--fill-strong)',
          display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: 'var(--label-alternative)' }}>
          {icon}
        </span>
      )}
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        {title != null && (
          <div style={{ fontSize: 15, fontWeight: 'var(--fw-bold)', color: 'var(--label-strong)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {title}
          </div>
        )}
        {(hasChips || ringCaption != null) && (
          <div style={{ marginTop: 3, fontSize: 12, fontWeight: 'var(--fw-semibold)', color: 'var(--label-alternative)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
      ...
      {ringLabel != null && (
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, flexShrink: 0, whiteSpace: 'nowrap',
          fontSize: 13, fontWeight: 'var(--fw-semibold)', letterSpacing: 0, color: 'var(--label-neutral)', fontVariantNumeric: 'tabular-nums' }}>
          {moving ? (
