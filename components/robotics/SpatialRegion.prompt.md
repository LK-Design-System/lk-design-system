...
- 선택·포커스·오류·지연은 서로 독립인 시각 상태입니다. 자동 접근성 이름에도 동일한 상태를 넣고 오류에는 `aria-invalid`를 노출합니다. passive selected/focused region은 `role="img"`를 유지하며 조작 상태인 `aria-pressed`를 만들지 않습니다. `disabled` opacity는 `0.45`, `stale` opacity는 `0.76`으로 지도 feature 형제와 일치시킵니다. 이 상태 opacity·상태 dash·label halo·상태 badge 값은 이제 내부 공용 `_navigationVocabulary` 모듈에서 가져오므로 형제 renderer와의 일관성이 수기 약속이 아니라 구조적으로 보장됩니다(넓은 구역을 그리는 region은 point hit-target을 두지 않습니다).
