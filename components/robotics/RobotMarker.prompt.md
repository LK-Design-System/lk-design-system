...
- pose의 body 색은 liveness만 전달합니다: 기본 accent, `stale`이면 muted + 점선 링, `invalid`이면 danger + exclamation glyph. identity와 heading은 색과 무관하게 label·노즈로 전달하므로 색만으로 상태를 전달하지 않습니다.
