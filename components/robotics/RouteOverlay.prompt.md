...
- `progress`는 source가 명시한 `{ segmentId, fraction, position? }`입니다. component는 phase, status, segment 개수로 전체 진행률을 계산하지 않습니다. `fraction`까지의 segment는 strong elapsed line으로 그리고 그 끝에 채움 삼각형 head 하나를 `marker-end`로 붙이며, 미래 선은 tip 앞을 `futureGap`만큼 비운 뒤 같은 tone의 recessed line으로 다시 시작합니다 — head의 tip이 실제 끝점으로 읽힙니다. 몸통 선은 tip보다 `tipSetback`만큼 일찍 멈추고 marker `refX`가 같은 값만큼 이동해, 둥근 stroke cap이 화살촉 안에 숨은 채 tip은 정확히 source 위치에 그려집니다. fraction 0에서는 elapsed 선이 없으므로 head도 그리지 않습니다(합성 carrier 부착부를 만들지 않습니다). `position`은 같은 fraction boundary의 정밀 좌표여야 합니다. fraction-derived anchor와의 거리가 화면에서 2 CSS px 이내일 때만 exact tip으로 사용하고, 초과하면 strong/recessed split과 접근성의 fraction은 보존하되 head·obstacle을 생략하고 `data-progress-position-mismatch="true"`를 남깁니다. 이 fail-closed 규칙은 source 좌표를 투영하거나 임의 연결선을 만들지 않습니다. `activeMapId` 필터가 segment를 제거하면 head, accessible name, `data-progress-*`도 함께 제거합니다.
