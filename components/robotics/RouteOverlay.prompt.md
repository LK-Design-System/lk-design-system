...
- `progress`는 source가 명시한 `{ segmentId, fraction, position? }`입니다. component는 phase, status, segment 개수로 전체 진행률을 계산하지 않습니다. `fraction`까지의 segment는 strong elapsed line으로 그리고 그 끝에 채움 삼각형 head 하나를 `marker-end`로 붙이며, 미래 선은 tip 앞을 `futureGap`만큼 비운 뒤 같은 tone의 recessed line으로 다시 시작합니다 — head의 tip이 실제 끝점으로 읽힙니다. 몸통 선은 tip보다 `tipSetback`만큼 일찍 멈추고 marker `refX`가 같은 값만큼 이동해, 둥근 stroke cap이 화살촉 안에 숨은 채 tip은 정확히 source 위치에 그려집니다. fraction 0에서는 elapsed 선이 없으므로 head도 그리지 않습니다(합성 carrier 부착부를 만들지 않습니다). `position`은 같은 fraction boundary의 정밀 좌표여야 합니다. fraction-derived anchor와의 거리가 화면에서 2 CSS px 이내일 때만 exact tip으로 사용하고, 초과하면 strong/recessed split과 접근성의 fraction은 보존하되 head·obstacle을 생략하고 `data-progress-position-mismatch="true"`를 남깁니다. 이 fail-closed 규칙은 source 좌표를 투영하거나 임의 연결선을 만들지 않습니다. `activeMapId` 필터가 segment를 제거하면 head, accessible name, `data-progress-*`도 함께 제거합니다. `showProgressHead={false}`는 `RobotMarker`가 progress 위치를 점유하는 합성에서 head를 로봇에게 양보하는 prop입니다 — strong/recessed 진행 대비는 유지하고 화살촉·shaft setback·future gap만 생략하며, 기본값은 `true`입니다.
...
route group 이름 다음에 각 interactive segment가 segment label → phase → condition 순으로 읽힙니다. 시각 paint order는 selection/focus halo → gap 이후의 recessed future segment → strong elapsed path + solid progress head → condition glyph → 별도 route lifecycle badge입니다. 단 기본 활동 상태 `active`는 lifecycle badge를 생략합니다 — solid 강조 선·진행 head·로봇이 이미 이동 중을 전달하므로 배지는 예외 상태 전용이며, 접근성 이름은 항상 상태를 포함합니다. Route는 segment마다 일반 방향 glyph를 반복하지 않습니다. ordered segment geometry와 label이 계획 경로의 순서를 보존하고, source-owned progress가 있을 때만 elapsed line 끝의 채움 화살촉이 현재 접선 방향을 한 번 전달합니다. progress head는 path anchor에 고정된 obstacle이며 head와 같은 path tangent로 회전하고 collision row로 이동하지 않습니다. condition/status/invalid/stale badge는 선을 가리지 않도록 자기 path anchor의 위쪽 화면 법선으로 `NAV_STATE_BADGE.pathNormalOffset`만큼 떠 있고(anchor `data-*` 좌표는 실제 path 지점 유지), 기본 오프셋일 때는 badge 링과 같은 톤의 `NAV_BADGE_LEADER` 틱이 badge를 자기 선에 묶으며(stale이어도 틱은 실선, screen-slot row는 틱 없음), 모든 badge·T chip은 공유 `NAV_MARKER_SHADOW` 캐스트 그림자로 지면에 접지합니다. head와의 실제 CSS 거리가 충돌하면 compact screen-space row로 이동하며, label은 별도 annotation row가 최소 4 CSS px 여백을 확보합니다(조건 badge가 있는 세그먼트의 기본 label은 badge 위로 올라감). 상태 badge는 최소 10 CSS px `NavigationStateGlyph`를 유지하며 progress head에는 circle, plate, shadow, lifecycle glyph를 넣지 않습니다. transition의 `T`는 상태 glyph가 아니라 topology metadata이므로 caption token text로 유지합니다.
...
| `WaypointMarker` / `LaneOverlay` | inverse scale, 24px target, focus/selection, `_navigationVectorGlyph` 어휘 | Lane의 선 절개 방향 셰브론은 정적 graph 방향이고 Route는 이를 쓰지 않음; Route는 source-owned solid progress head(`NAV_PROGRESS_TRIANGLE`)만 사용 |
...
- [Mapbox Navigation Route Arrow](https://docs.mapbox.com/android/navigation/guides/ui-components/route-arrow/)와 [Google Maps polyline symbols](https://developers.google.com/maps/documentation/javascript/symbols)는 방향 표식을 line의 끝/offset에 결합하고 local line direction으로 정렬합니다. Mapbox의 custom arrow head는 마지막 두 점의 bearing으로 결정되므로 LDS도 current progress head를 elapsed line의 마지막 접선에 결합합니다. Route segment마다 별도 방향 glyph를 반복하지 않으며 외부 styling은 복제하지 않습니다.
- [TomTom route display](https://developer.tomtom.com/navigation/android/guides/map-display/map-display-for-views/routes)는 traveled route progress와 route-bound maneuver arrow를 독립 정보로 둡니다. LDS Route는 source-owned progress line/head만 현재 위치·접선을 표현하고 lifecycle은 별도 badge로 유지하며, 일반 방향 glyph를 추가로 중첩하지 않습니다.
- [W3C SVG Markers](https://www.w3.org/TR/svg-markers/)의 `marker-end`와 `orient="auto"`가 채움 화살촉 tip을 source position과 incoming tangent에 결합하는 구현 근거입니다. Route에서는 이 source-owned endpoint marker가 유일한 방향 cue이며, 화살촉이 자체 surface 외곽선을 갖기 때문에 별도 casing marker 없이 marker 정의 하나로 렌더됩니다.
...
- dense sample interpolation, heading/time, robot pose/footprint(`RobotMarker` 소관)
