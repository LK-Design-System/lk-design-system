...
- path는 `vector-effect="non-scaling-stroke"`, 투명 hit path는 24 CSS px이며 status 부근의 34px 원형 core가 짧은 궤적에서도 24×24 CSS px 정사각형을 보장합니다. marker/label은 inverse `viewportScale`을 사용합니다.
...
`invalid`와 `stale`은 각각 `!`와 `~` screen-space glyph를 추가해 negative color나 opacity만으로 전달하지 않습니다. 둘이 함께 있으면 서로 다른 path 위치에 배치하며 trajectory status pattern은 그대로 보존합니다.
...
- [MapLibre Style Spec — line and symbol layers](https://maplibre.org/maplibre-style-spec/layers/)는 line geometry와 symbol collision/priority를 별도 설정으로 둡니다. LDS는 current/status/validation glyph만 제공하며 전체 sample label 배치와 충돌 해결을 owning renderer에 남깁니다.
- [WCAG 2.2 Target Size (Minimum)](https://www.w3.org/WAI/WCAG22/Understanding/target-size-minimum.html)은 24×24 CSS px 정사각형이 target 내부에 들어가야 한다고 설명합니다. path stroke만으로 부족할 수 있는 짧은 trajectory를 midpoint core가 보완합니다.
