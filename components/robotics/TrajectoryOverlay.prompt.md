...
- status는 planned/active/waiting/blocked/rerouting/completed path pattern과 별도 lifecycle badge glyph를 결정합니다. 단 기본 활동 상태 `active`는 배지를 생략합니다 — solid 강조 선·진행 head·로봇이 이미 이동 중을 전달하므로 배지는 예외 상태 전용이며, 접근성 이름은 항상 상태를 포함합니다. full path의 pattern을 recessed future에도 유지하고, strong elapsed line은 progress 연결성을 위해 solid로 그립니다. Status/validation hue는 line과 badge outline에 유지합니다.
