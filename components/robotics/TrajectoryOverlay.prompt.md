...
- `currentSampleIndex`는 source가 명시한 현재 sample입니다. phase나 시간에서 추론하지 않습니다. 해당 sample까지의 line은 strong, 이후는 같은 tone의 recessed line이며 open `marker-end`는 ordered path의 incoming tangent를 따릅니다. index 0에서 쓰는 16 CSS px carrier는 elapsed sample을 합성한 것이 아니라 head를 시작점에 결합하는 최소 시각 부착부입니다. `headingRad`는 public source data로 보존하지만 progress head를 회전시키지 않습니다. robot bearing·pose·footprint는 별도 renderer가 소유합니다.
...
- 자산 suitability inventory에서 `assets/icons/pause.svg`, `close.svg`, `refresh.svg`, `check.svg`, `exclamation.svg`, `question.svg`, `clock.svg`와 생성된 `Icon` registry를 먼저 확인했습니다. lifecycle/validation/fallback은 registry path를 유지하고 current progress는 Route와 같은 `NAV_PROGRESS_HEAD` open geometry를 사용합니다.
...
| `WaypointMarker` | inverse scale, 24px target, focus/selection/disabled/invalid/stale | dense polyline과 path-bound current progress head가 필요함; robot pose는 제외 |
...
- [W3C SVG Markers](https://www.w3.org/TR/svg-markers/)의 `marker-end`와 `orient="auto"`가 open V를 incoming path tangent에 정렬하는 구현 근거입니다.
