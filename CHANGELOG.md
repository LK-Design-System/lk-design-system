...
- Route/Trajectory status badges are now exception-state chrome: the default working status (`active`) renders no lifecycle badge — the strong accent line, progress head, and robot already say "moving", so the ▶ chip was informationless clutter. Planned/waiting/blocked/rerouting/completed and the independent invalid/stale badges are unchanged, and accessible names always include the status. The composed Navigation Viewer also delegates the lane's on-map label/metadata to the LayerPanel row (`showLabel={false}`), removing duplicated map text.
- Added `showProgressHead` (default `true`) to `RouteOverlay` and `TrajectoryOverlay`: a composition that places a `RobotMarker` on the progress position passes `false` so the arrowhead, its shaft setback, and the future gap yield to the robot while the strong/recessed progress split stays — the head and the robot otherwise draw the same "current position" twice. The Navigation Viewer scene wires it to the robot layer's visibility.
...
- Navigation scene cohesion: every floating point marker (pins, the waypoint diamond, state/condition badges, T/count/endpoint chips, the robot body) now casts the shared `NAV_MARKER_SHADOW` silhouette shadow so markers read as standing on one ground plane; default-offset Route/Trajectory badges draw a ring-tone `NAV_BADGE_LEADER` tick tethering them to their line; and an available lane retones from accent to `--viewer-muted` so static topology stays quiet while live state (current route, active trajectory, robot) owns the accent — the figure/ground hierarchy the composed viewer lacked. The Navigation Viewer scene itself was recomposed: the lift-lobby region moved inside the frame and now contains the corridor endpoints and facility pin, the keep-out zone sits inside the corridor elbow so the bend reads as detouring around it, and a `RobotMarker` derived from the trajectory's current sample joins the scene with its own layer-panel group and legend row.
