...
export function SpeedDial({ icon, actions = [], open, defaultOpen = false, onOpenChange, label = '작업', style, onKeyDown, ...rest }) {
  ...
  return (
    ...
    >
      ...
      {isOpen && (
        <ul id={listId} style={{ listStyle: 'none', margin: 0, padding: 0, display: 'grid', gap: 'var(--space-2-5)', justifyItems: 'end' }}>
          {actions.map((a, i) => {
            const actionLabel = a.ariaLabel || (typeof a.label === 'string' ? a.label : undefined);
            const actionId = `${dialId}-action-${i}`;
            return (
              <li key={actionLabel ?? i} style={{ display: 'inline-flex', alignItems: 'center', gap: 'var(--space-2-5)' }}>
