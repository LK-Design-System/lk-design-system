...
function interactionTone(base, hover, { hovered, pressed }) {
  ...
  return hovered
    ? `color-mix(in srgb, ${hover || base} 96%, var(--color-semantic-label-normal))`
    : base;
}
...
/** A dominant action joined to a menu button for closely related alternatives. */
  ...
  return (
    ...
    >
      <button
        ...
        onMouseLeave={() => { setMainHover(false); setMainPressed(false); }}
        onMouseDown={() => { if (!blocked) setMainPressed(true); }}
        ...
        style={{
          ...
          background: blocked
            ...
            : interactionTone(palette.bg, palette.hover, { hovered: mainHover, pressed: mainPressed }),
        }}
      >
        {loading && (
          <span aria-hidden="true" style={{ position: 'absolute', inset: 0, display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
            <Spinner size={sizeStyle.iconSize} color="currentColor" />
          </span>
        )}
        <span aria-hidden={loading || undefined} style={{ visibility: loading ? 'hidden' : undefined }}>
          {children}
        </span>
      </button>
      <button
        ...
        onMouseLeave={() => { setMenuHover(false); setMenuPressed(false); }}
        onMouseDown={() => { if (!blocked) setMenuPressed(true); }}
        ...
        style={{
          ...
          background: blocked
            ? 'var(--component-button-disabled-bg)'
            : interactionTone(palette.bg, palette.hover, { hovered: menuHover, pressed: menuPressed }),
          display: 'inline-flex',
          alignItems: 'center',
          justifyContent: 'center',
        }}
      >
        <Icon name="chevron-down-small" size={sizeStyle.iconSize} aria-hidden="true" />
      </button>
      {open && (
        <div
          ref={(node) => {
        ...
        >
          {items.map((item, index) => (
            <SplitMenuItem
              key={item.value ?? (typeof item.label === 'string' ? item.label : index)}
              item={item}
