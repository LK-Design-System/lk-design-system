...
export function NavRail({ items = [], value, defaultValue, onChange, renderLink, style, ...rest }) {
  ...
  return (
    <nav aria-label="주 탐색" style={{ display: 'inline-flex', flexDirection: 'column', width: 'fit-content', maxWidth: '100%', boxSizing: 'border-box', gap: 'var(--space-1-5)', padding: 'var(--space-2-5)', background: 'var(--color-semantic-background-elevated-normal)', border: '1px solid var(--color-semantic-line-solid-normal)', borderRadius: 'var(--radius-xl)', ...style }} {...rest}>
      {items.map((o) => {
