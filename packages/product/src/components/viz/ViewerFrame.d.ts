...
export type ViewerConnection = 'connecting' | 'connected' | 'disconnected' | 'no-signal';
export type ViewerFreshness = 'current' | 'degraded' | 'stale' | 'frozen';
export type ViewerPlayback = 'playing' | 'live' | 'paused';
...
export interface ViewerFrameProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  /** Legacy combined presentation state. Prefer the orthogonal state axes for new product code. @default "ready" */
  state?: ViewerState;
  /** Source/content availability. Explicit axes take precedence over `state`. @default "ready" */
  availability?: ViewerAvailability;
  /** Transport connection truth. @default "connected" */
  connection?: ViewerConnection;
  /** Freshness of the last rendered content. @default "current" */
  freshness?: ViewerFreshness;
  /** Playback/live presentation mode. @default "playing" */
  ...
  /** Chrome treatment. "overlay" uses compact translucent edge surfaces over visual media. @default "surface" */
  chromeVariant?: 'surface' | 'overlay';
  /** Toolbar reveal behavior. "interaction" shows controls while the frame is hovered, pressed, or contains focus. @default "always" */
