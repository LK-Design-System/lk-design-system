...
/* The track slide and the dot resize are inline transitions, so a plain
   `@media (prefers-reduced-motion: reduce)` rule would lose to them — the
...
const rotationBtnStyle = { position: 'absolute', left: 'var(--space-3)', bottom: 'var(--space-2)', width: 32, height: 32, borderRadius: '50%', border: 'none', background: 'var(--scrim-dark)', color: 'var(--color-semantic-static-white)', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(4px)', zIndex: 2 };

/* 24x24 hit area around an 8px-tall pill: the visible dot stays a dot while the
   target meets WCAG 2.5.8. */
const dotBtnStyle = { width: 24, height: 24, padding: 0, border: 'none', background: 'none', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' };
...
export function Carousel({
  ...
  return (
    <div
      ...
      onFocus={(event) => { onFocus?.(event); setHeld(true); }}
      onBlur={(event) => {
        onBlur?.(event);
    ...
    >
      ...
      {showDots && n > 1 && (
        <div style={{ position: 'absolute', bottom: 'var(--space-2)', left: 0, right: 0, display: 'flex', justifyContent: 'center', zIndex: 2 }}>
          {slides.map((_, idx) => (
            <button
              key={idx}
              type="button"
