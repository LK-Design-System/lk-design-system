...
패키지 진입 파일은 현재 173개의 React 컴포넌트 소스 파일(`components/**/*.jsx`) 기준으로 생성됩니다.
...
현재 이 저장소의 public Storybook sidebar는 `LDS Core/Foundation`, `LDS Core/Components`, `LDS Theme`, `LDS Product` 아래의 컴포넌트와 패턴 표면 304개 story를 다룹니다. `LDS Robotics` Storybook은 `LK Design System Robotics` 저장소가 별도로 소유합니다.
...
이 패키지는 다음 그룹에 걸쳐 173개의 React 컴포넌트 소스 파일을 export합니다.
