...
패키지 진입 파일은 현재 207개의 React 컴포넌트 소스 파일(`components/**/*.jsx`) 기준으로 생성됩니다.
...
현재 이 저장소의 public Storybook sidebar는 `LDS Core/Foundation`, `LDS Core/Components`, `LDS Theme`, `LDS Product` 아래의 컴포넌트와 패턴 표면 487개 story를 다룹니다. `LDS Robotics` Storybook은 `LK Design System Robotics` 저장소가 별도로 소유합니다.
...
WDS 원본 및 실제 확장 컴포넌트 회귀를 비교하기 위한 91개 `visual-parity` story는 direct iframe 검증용으로만 남기고 `!dev` 태그로 sidebar에서 숨깁니다. `npm run check:storybook-public`이 public 중복 노출과 parity story 노출을 차단합니다.
...
이 패키지는 다음 그룹에 걸쳐 207개의 React 컴포넌트 소스 파일을 export합니다.
