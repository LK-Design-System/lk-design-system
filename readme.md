...
현재 이 저장소의 public Storybook sidebar는 `LDS Core/Foundation`, `LDS Core/Components`, `LDS Core/Patterns`, `LDS Theme`, `LDS Product` 아래의 컴포넌트와 패턴 표면 381개 story를 다룹니다. `LDS Robotics` Storybook은 `LK Design System Robotics` 저장소가 별도로 소유합니다.
