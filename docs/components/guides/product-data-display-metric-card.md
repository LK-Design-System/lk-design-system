...
| 명시 규칙 4 | 기존 uppercase caption label, title1 value, legacy delta color는 호환합니다. label은 --ls-overline tracking을 쓰지 않고 letter-spacing: 0으로 두어 한국어가 음절 단위로 벌어지지 않게 합니다. header icon은 장식 맥락이므로 중립 --color-semantic-label-alternative를 상속하며, 상태 색이 필요하면 host가 icon 자체에 색을 지정합니다(MetricCard에는 icon tone prop이 없습니다). |
| --body2-line | 22px |
