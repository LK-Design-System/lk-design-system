...
| `tone` | `'signal' \| 'steel' \| 'amber' \| 'moss' \| 'navy'` | No | 아이콘 타일 톤. @default "signal" |
...
| Subject | Rule |
| --- | --- |
| 명시 규칙 1 | tone — signal(틸 타일, 기본) · steel · amber · moss · navy. steel은 signal과 같은 파랑 계열이므로 카드 셋 이상을 나란히 두고 색으로 구분하려면 signal·amber·moss처럼 색상(hue)이 다른 톤을 고르세요. moss와 amber는 전용 surface 토큰 없이 전경색의 14% 틴트를 타일 배경으로 파생합니다. |
| 명시 규칙 2 | density — comfortable(기본) · compact. Compact는 boxed padding을 --space-4(16px), icon-to-content gap을 --space-3(12px), icon tile을 --space-10(40px), title-to-description gap을 --space-1(4px)로 조정합니다. 제목과 본문 typography는 바꾸지 않으며 루트에 data-density를 노출합니다. |
| 명시 규칙 3 | headingLevel — title은 실제 heading으로 렌더되며 기본 레벨은 h4입니다(Core Card의 구조화 title은 h3). 카드가 놓인 문서의 제목 계층에 맞춰 1–6을 주고, 제목이 이미 카드 바깥에 있으면 headingLevel={false}로 heading 의미를 끄세요. 레벨은 건너뛰지 않습니다(WCAG 1.3.1). |
| 명시 규칙 4 | 카드 전체 클릭 — onClick을 주면 카드 루트가 role="button" · tabIndex=0이 되고 Enter/Space로 활성화됩니다(WCAG 2.1.1). 포커스 링은 토큰 레이어(tokens/focus.css)의 전역 :focus-visible 규칙이 담당하므로 카드가 따로 그리지 않습니다. 이때 카드 안에 버튼·링크 같은 포커스 가능한 요소를 넣지 마세요 — 중첩 인터랙티브는 유효하지 않은 마크업이고 카드 이름이 본문 전체로 길어집니다. 행동이 여러 개면 onClick 없이 각 액션을 카드 안에 두세요. |
| --body2-size | 15px |
