...
- WAI-ARIA Disclosure Pattern — 그룹 트리거는 button으로 동작하고 aria-expanded가 실제 자식 콘텐츠의 표시 상태를 반영해야 합니다. single 정책은 이 동일한 disclosure semantics 위에 공간 절약형 상호 배타 상태만 추가하며, 키보드 Enter/Space와 포커스 계약은 변경하지 않습니다.
...
| `groupExpansionMode` | `'multiple' \| 'single'` | No | 그룹 disclosure 정책. multiple은 여러 그룹을 동시에 열고, single은 최대 한 그룹만 엽니다. @default 'multiple' |
| `expandedGroupValues` | `string[]` | No | 제어되는 열린 그룹 값 목록. 지정하면 SideNav는 상태를 직접 바꾸지 않고 변경 요청만 알립니다. |
| `defaultExpandedGroupValues` | `string[]` | No | 비제어 열린 그룹의 초기 값 목록. @default [] |
| `onExpandedGroupValuesChange` | `(values: string[], changedValue: string, expanded: boolean) = void` | No | 열린 그룹 변경 요청. changedValue는 토글된 그룹이고 expanded는 요청된 다음 상태입니다. |
...
| expandedGroupValues | 제어되는 열린 그룹 값 목록. 지정하면 SideNav는 상태를 직접 바꾸지 않고 변경 요청만 알립니다. |
| defaultExpandedGroupValues | 비제어 열린 그룹의 초기 값 목록. @default [] |
| onExpandedGroupValuesChange | 열린 그룹 변경 요청. changedValue는 토글된 그룹이고 expanded는 요청된 다음 상태입니다. |
...
- Windows Expander guidance — 여러 확장 영역에서 한 번에 하나만 열도록 하는 정책은 선택 가능한 편의 기능이어야 하며 사용자가 다시 열림을 제어할 수 있어야 합니다. 그래서 기본값은 기존 multiple로 유지하고 single을 명시적 opt-in으로 제공합니다.
...
| 명시 규칙 2 | groupExpansionMode / expandedGroupValues / defaultExpandedGroupValues / onExpandedGroupValuesChange — 기본 multiple은 기존처럼 여러 disclosure 그룹을 동시에 엽니다. single은 한 번에 최대 한 그룹만 열고, 다른 그룹을 열면 이전 그룹을 닫으며 열린 그룹을 다시 누르면 접습니다. |
