...
| Part | Contract |
| --- | --- |
| Root | FileUploadQueue의 semantic role, layout containment와 전달받은 DOM 속성을 소유합니다. |
...
| --- | --- | --- | --- |
| `items` | `FileUploadQueueItem[]` | No | 공개 타입 계약에 정의된 속성입니다. |
...
| `onRetry` | `(item: FileUploadQueueItem) = void` | No | 공개 타입 계약에 정의된 속성입니다. |
| `onCancel` | `(item: FileUploadQueueItem) = void` | No | 업로드·처리 중인 행의 명시적 취소 요청. |
| `onRemove` | `(item: FileUploadQueueItem) = void` | No | 공개 타입 계약에 정의된 속성입니다. |
| `onOpen` | `(item: FileUploadQueueItem) = void` | No | 공개 타입 계약에 정의된 속성입니다. |
...
| onOpen | 공개 타입 계약에 정의된 속성입니다. 타입 계약: (item: FileUploadQueueItem) = void |
| 변형·상태 · 대기 · 업로드와 빈 상태 | variants-states 공개 스토리에서 렌더링 및 상호작용 증거를 유지합니다. |
| 변형·상태 · 미디어 첨부(썸네일) | variants-states 공개 스토리에서 렌더링 및 상호작용 증거를 유지합니다. |
| 반응형 · 좁은 폭의 진행과 동작 | responsive 공개 스토리에서 렌더링 및 상호작용 증거를 유지합니다. |
...
| Subject | Rule |
| --- | --- |
| 명시 규칙 1 | 개별 상태 badge는 파일명 바로 옆의 정보 영역에 두고, 오른쪽 trailing은 action만 소유합니다. busy 행은 6px progress track과 tabular percentage를 같은 줄에 배치합니다. |
...
| 명시 규칙 3 | - 파일 선택 dropzone에는 FileUpload을 사용합니다. - uploading과 processing을 구분해 전송 완료를 최종 완료처럼 표시하지 않습니다. - layout — 항목을 어떻게 보여줄지만 고릅니다. 상태 어휘·진행률·재시도/취소/제거 의미와 접근 이름은 두 값이 동일합니다. - list(기본) — 문서·리포트용. 파일은 개별 카드가 아니라 하나의 queue 안에서 divider 행으로 표시하고, 아이콘 · 이름 · 상태 · 크기를 읽는 순서로 둡니다. - grid — 사진·영상 첨부용 스트립. 정사각 썸네일을 한 줄로 깔고 넘치면 wrap합니… |
...
- - 파일 선택 dropzone에는 FileUpload을 사용합니다. - uploading과 processing을 구분해 전송 완료를 최종 완료처럼 표시하지 않습니다. - layout — 항목을 어떻게 보여줄지만 고릅니다. 상태 어휘·진행률·재시도/취소/제거 의미와 접근 이름은 두 값이 동일합니다. - list(기본) — 문서·리포트용. 파일은 개별 카드가 아니라 하나의 queue 안에서 divider 행으로 표시하고, 아이콘 · 이름 · 상태 · 크기를 읽는 순서로 둡니다. - grid — 사진·영상 첨부용 스트립. 정사각 썸네일을 한 줄로 깔고 넘치면 wrap합니….

## Do / Don't

| Kind | Guidance |
| --- | --- |
| Do | 파일 선택 dropzone에는 FileUpload을 사용합니다. |
| Don't | uploading과 processing을 구분해 전송 완료를 최종 완료처럼 표시하지 않습니다. |
| Do | layout — 항목을 어떻게 보여줄지만 고릅니다. 상태 어휘·진행률·재시도/취소/제거 의미와 접근 이름은 두 값이 동일합니다. |
| Don't | grid — 사진·영상 첨부용 스트립. 정사각 썸네일을 한 줄로 깔고 넘치면 wrap합니다. 미디어는 파일명보다 그림으로 식별되므로 행 목록이 아니라 타일이고, 파일명·상태 텍스트를 화면에 쌓지 않습니다(대신 타일마다 시각적으로 숨긴 이름, 상태 텍스트를 둬 보조기기에는 그대로 전달합니다). |

## Exceptions

- 제품 정책을 포함해야 하는 예외는 wrapper 또는 제품 저장소에서 조합하고 FileUploadQueue의 범용 API에 넣지 않습니다.
...
- [seed-design Attachment Input](https://seed-design.io/components/attachment-input)
