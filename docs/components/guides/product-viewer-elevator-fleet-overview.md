...
여러 건물의 엘리베이터 위치와 운행 상태를 한 번에 비교해야 할 때 사용합니다. 한 대의 상세 제어, 호출 입력, 시간 추세 분석에는 사용하지 않고 해당 전용 화면으로 이동합니다. 같은 건물의 엘리베이터는 하나의 층 눈금을 공유하고, 건물 그룹은 좌우로 이어 붙습니다. 건물마다 층 구성이 다르므로 건물 사이의 같은 층 이름을 같은 높이에 맞추지 않습니다.
