...
| Part | Contract |
| --- | --- |
| actionContext | Accessible context prefixed to reveal/copy action names. Required to distinguish complex ReactNode labels; pass false when action label props already contain their complete accessible names. |
| revealLabel | Action text. The visible field label is added to the accessible button name. |
| hideLabel | Action text. The visible field label is added to the accessible button name. |
| copyLabel | Action text. The visible field label is added to the accessible button name. |
| copiedLabel | Success text used by both the button name and live feedback. |
| copyErrorLabel | Failure text used by both the button name and live feedback. |
...
| `revealable` | `boolean` | No |  |
| `copyable` | `boolean` | No |  |
| `revealDurationMs` | `number` | No |  |
| `revealed` | `boolean` | No |  |
| `defaultRevealed` | `boolean` | No |  |
| `onRevealChange` | `(revealed: boolean) = void` | No |  |
| `onCopy` | `(value: string) = void` | No |  |
| `onCopyError` | `(error: unknown) = void` | No |  |
...
| --color-semantic-status-positive-foreground | light: var(--color-atomic-green-40); dark: var(--color-semantic-status-positive-signal) |
| --space-0 | 0px |
...
- GOV.UK Password input에서 hidden-by-default, 명시적인 show/hide action, 여러 field에서 구분되는 toggle label, spellcheck=false와 autocapitalize=off 기준을 확인했습니다.
- PatternFly Clipboard copy accessibility는 copy action 이름을 field 맥락과 연결하고 성공 뒤 feedback을 갱신하도록 권장합니다. 이를 contextual button name과 live status로 번역했습니다.
