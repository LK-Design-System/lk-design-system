...
| `RouteOverlay` / `TrajectoryOverlay` | Route는 층별 graph segment phase·condition과 명시적 segment progress를, Trajectory는 한 map의 ordered position/time/heading sample을 소유한다. 둘의 현재 진행 위치는 strong elapsed line + open `marker-end`로 통일하되 lifecycle badge와 robot pose/bearing/footprint는 별도 정보 계층으로 유지한다. route와 path를 같은 데이터 계약으로 합치거나 층 사이를 임의 직선으로 연결하지 않는다. |
