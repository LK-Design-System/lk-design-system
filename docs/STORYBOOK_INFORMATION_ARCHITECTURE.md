...
Current generated census (2026-08-04): 192개 페이지와 675개 스토리. The machine-readable audit is authoritative for public/hidden visibility and review status.

- 공개 스토리: 465개
- 숨김 스토리: 210개
- 숨김 visual parity: 90개
- 숨김 internal contract: 116개
- 검토 완료 페이지: 192/192개
- 검토 완료 스토리: 675/675개
...
2026-08-04 현재 소스에서 재생성한 정적 빌드 기준으로 192개 페이지와 675개 스토리가 있다. census 판정은 모두 `keep`다.

- 공개 스토리: 465개
- 숨김 스토리: 210개
- 숨김 visual parity: 90개
- 숨김 internal contract: 116개
- 검토 완료 페이지 192/192개, 스토리 675/675개다. 변경된 기존 페이지의 stale source hash와 신규 스토리 검토는 감사 원장에 그대로 남겨 두며, `check:storybook-ia`가 소유권·공개 여부·이름·설명과 함께 회귀를 막는다.
...
- 생성기의 generic fallback을 삭제하고 누락된 근거를 `omitted-no-evidence`로 드러냈다. 현재 174개 가이드의 평균 중복 prose는 3%, 60% 초과 페이지는 0개이며 `STORYBOOK_GUIDE_DEDUP_BASELINE.json`이 이 값을 lowering-only ratchet으로 고정한다. 토큰 값·API·property 설명은 prose 지표에서 제외하고 실제 정량 규칙은 포함한다. 같은 소유자의 별도 페이지는 문구를 복사하지 않고 Dashboard Shell → Dashboard Navigation, Brand Spinner → Core Spinner처럼 한 단계 canonical reference와 근거가 있는 local delta만 기록한다. Foundation 16페이지의 손으로 쓴 가이드는 페이지별 고유 원리·제약을 그대로 유지한다.

2026-07-26 검토에서는 운영용 `Overview`·`Progress Board`를 정적 문서로 옮기고 Loading을 교차 컴포넌트 패턴 Docs로 연결했다. 이후 현재 192개 페이지와 675개 스토리의 owner, role, visibility, disposition은 감사 원장을 따른다.

- 컴포넌트 설명이 있는 페이지: 188개
- Canvas에서 안내 서문을 직접 보여 주는 페이지: 188개
- 사용/비사용 판단 기준이 확인되는 페이지: 181개(Foundation 페이지는 사용 판단 대신 원리·제약 계약을 적용할 수 있음)
- 목적 설명이 있는 공개 스토리: 465개
- Foundation: 16페이지, 22스토리(공개 17·숨김 5)
- Core: 57페이지, 204스토리(공개 113·숨김 91)
- Product: 115페이지, 440스토리(공개 330·숨김 110)
...
- 공개 내부 계약을 `!dev`로 전환하고 Communication·VirtualKeypad의 실제 컴포넌트 회귀 표면을 정리해 현재 visual parity는 90개다.

최종 구조에서는 분리된 각 페이지와 후속 확장 페이지가 독립 소유자를 가지므로 현재 census 판정은 `keep` 192개, `split` 0개, `merge` 0개, `hide` 0개다. 검토 완료 수와 stale source hash는 감사 원장의 현재 summary를 기준으로 하며, `check:storybook-ia`가 이 상태를 회귀 방지한다.
