...
Current generated census (2026-08-02): 191개 페이지와 669개 스토리. The machine-readable audit is authoritative for public/hidden visibility and review status.

- 공개 스토리: 462개
- 숨김 스토리: 207개
- 숨김 visual parity: 89개
- 숨김 internal contract: 114개
- 검토 완료 페이지: 191/191개
- 검토 완료 스토리: 669/669개
...
2026-08-02 현재 소스에서 재생성한 정적 빌드 기준으로 191개 페이지와 669개 스토리가 있다. census 판정은 모두 `keep`다.

- 공개 스토리: 462개
- 숨김 스토리: 207개
- 숨김 visual parity: 89개
- 숨김 internal contract: 114개
- 검토 완료 페이지 191/191개, 스토리 669/669개다. 변경된 기존 페이지의 stale source hash와 신규 스토리 검토는 감사 원장에 그대로 남겨 두며, `check:storybook-ia`가 소유권·공개 여부·이름·설명과 함께 회귀를 막는다.
- Component 문서 시스템은 public component entry 204개와 named export 216개를 전수 추적하고, 173개 evidence-only 결정 가이드를 `docs/components/guides/*.md`와 `llms.txt`로 생성한다. Storybook의 Docs 탭에는 사용 판단 · Properties · Accessibility · Tokens만 렌더링한다. 전체 카탈로그와 플랫폼 현황은 Storybook 운영 화면이 아니라 생성 문서 `COMPONENT_REFERENCE.md`와 `PROGRESS_BOARD.md`가 소유한다.
...
- 생성기의 generic fallback을 삭제하고 누락된 근거를 `omitted-no-evidence`로 드러냈다. 현재 173개 가이드의 평균 중복 prose는 3%, 60% 초과 페이지는 0개이며 `STORYBOOK_GUIDE_DEDUP_BASELINE.json`이 이 값을 lowering-only ratchet으로 고정한다. 토큰 값·API·property 설명은 prose 지표에서 제외하고 실제 정량 규칙은 포함한다. 같은 소유자의 별도 페이지는 문구를 복사하지 않고 Dashboard Shell → Dashboard Navigation, Brand Spinner → Core Spinner처럼 한 단계 canonical reference와 근거가 있는 local delta만 기록한다. Foundation 16페이지의 손으로 쓴 가이드는 페이지별 고유 원리·제약을 그대로 유지한다.

2026-07-26 검토에서는 운영용 `Overview`·`Progress Board`를 정적 문서로 옮기고 Loading을 교차 컴포넌트 패턴 Docs로 연결했다. 이후 현재 191개 페이지와 669개 스토리의 owner, role, visibility, disposition은 감사 원장을 따른다.

- 컴포넌트 설명이 있는 페이지: 187개
- Canvas에서 안내 서문을 직접 보여 주는 페이지: 187개
- 사용/비사용 판단 기준이 확인되는 페이지: 180개(Foundation 페이지는 사용 판단 대신 원리·제약 계약을 적용할 수 있음)
- 목적 설명이 있는 공개 스토리: 462개
- Foundation: 16페이지, 22스토리(공개 17·숨김 5)
- Core: 57페이지, 204스토리(공개 113·숨김 91)
- Product: 114페이지, 433스토리(공개 326·숨김 107)
...
초기 전수 검토 패스에서 148개 페이지와 419개 스토리를 소스 AST와 빌드 인덱스에 대조했다. 최초 후보 18개 분리와 1개 병합은 `.fig`의 각 COMPONENT_SET 정의, LDS 공개 API, 스토리 상호작용 계약을 다시 확인한 뒤 다음처럼 수습했고, 후속 domain expansion 페이지도 같은 role·visibility·description 계약으로 검토했다.
...
- 공개 내부 계약을 `!dev`로 전환하고 Communication·VirtualKeypad의 실제 컴포넌트 회귀 표면을 정리해 현재 visual parity는 89개다.

최종 구조에서는 분리된 각 페이지와 후속 확장 페이지가 독립 소유자를 가지므로 현재 census 판정은 `keep` 191개, `split` 0개, `merge` 0개, `hide` 0개다. 검토 완료 수와 stale source hash는 감사 원장의 현재 summary를 기준으로 하며, `check:storybook-ia`가 이 상태를 회귀 방지한다.
...
순서는 `문제 → 권위·적용 범위 → 핵심 선택 기준 → 컴포넌트 조합 → 상태·실패 처리 → 접근성`이다.

- 첫 Canvas에서 `LDS Core · Pattern`과 적용되는 제품 범위를 명시한다.
- 단일 컴포넌트의 API가 아니라 여러 컴포넌트 사이의 선택과 조합을 설명한다.
- 개요는 핵심 규칙·결정 기준·조합 대상까지만 보여 주고, 선택기와 복합 상호작용은 별도 스토리로 분리한다.
- 정상 흐름뿐 아니라 완료·실패·재시도와 장기 작업의 이탈 경로를 연결한다.
- 제품의 timeout, 데이터 정책, 권한과 업무 규칙을 LDS 기본값으로 대신하지 않는다.
