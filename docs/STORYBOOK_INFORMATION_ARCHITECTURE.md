...
2026-07-27 현재 소스에서 재생성한 정적 빌드 기준으로 166개 페이지와 534개 스토리가 있다. census 판정은 모두 `keep`다.

- 공개 스토리: 379개
- 숨김 스토리: 155개
- 숨김 visual parity: 80개
- 숨김 internal contract: 72개
- 검토 완료 페이지 165/166개, 스토리 527/534개다. 변경된 기존 페이지의 stale source hash와 신규 스토리 검토는 감사 원장에 그대로 남겨 두며, `check:storybook-ia`가 소유권·공개 여부·이름·설명과 함께 회귀를 막는다.
...
2026-07-26 검토에서는 운영용 `Overview`·`Progress Board`를 정적 문서로 옮기고 Loading을 교차 컴포넌트 패턴 Docs로 연결했다. 이후 현재 166개 페이지와 534개 스토리의 owner, role, visibility, disposition은 감사 원장을 따른다.

- 컴포넌트 설명이 있는 페이지: 162개
- Canvas에서 안내 서문을 직접 보여 주는 페이지: 162개
- 사용/비사용 판단 기준이 확인되는 페이지: 154개(Foundation 페이지는 사용 판단 대신 원리·제약 계약을 적용할 수 있음)
- 목적 설명이 있는 공개 스토리: 378개
- Foundation 진입 가이드: 16/16페이지, 공개 스토리 설명: 16/16개
- Core 진입 가이드: 52/54페이지, 공개 스토리 설명: 104/104개
- Product 진입 가이드: 93/93페이지, 공개 스토리 설명: 255/256개
...
- 공개 내부 계약 10개를 `!dev`로 전환하고 Communication·VirtualKeypad의 실제 컴포넌트 회귀 표면을 정리해 현재 visual parity는 80개다.

최종 구조에서는 분리된 각 페이지와 후속 확장 페이지가 독립 소유자를 가지므로 현재 census 판정은 `keep` 166개, `split` 0개, `merge` 0개, `hide` 0개다. 검토 완료 수와 stale source hash는 감사 원장의 현재 summary를 기준으로 하며, `check:storybook-ia`가 이 상태를 회귀 방지한다.
