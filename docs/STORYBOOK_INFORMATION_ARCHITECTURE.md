...
Current generated census (2026-08-13): 195개 페이지와 720개 스토리. The machine-readable audit is authoritative for public/hidden visibility and review status.

- 공개 스토리: 496개
- 숨김 스토리: 224개
- 숨김 visual parity: 95개
- 숨김 internal contract: 125개
- 검토 완료 페이지: 195/195개
- 검토 완료 스토리: 707/720개
...
2026-08-13 현재 소스에서 재생성한 정적 빌드 기준으로 195개 페이지와 720개 스토리가 있다. census 판정은 모두 `keep`다.

- 공개 스토리: 496개
- 숨김 스토리: 224개
- 숨김 visual parity: 95개
- 숨김 internal contract: 125개
- 검토 완료 페이지 195/195개, 스토리 707/720개다. 페이지별 source hash와 신규 스토리 검토는 감사 원장에 남기며, `check:storybook-ia`가 소유권·공개 여부·이름·설명과 함께 회귀를 막는다.
...
2026-07-26 검토에서는 운영용 `Overview`·`Progress Board`를 정적 문서로 옮기고 Loading을 교차 컴포넌트 패턴 Docs로 연결했다. 이후 현재 195개 페이지와 718개 스토리의 owner, role, visibility, disposition은 감사 원장을 따른다.
...
최종 구조에서는 분리된 각 페이지와 후속 확장 페이지가 독립 소유자를 가지므로 현재 census 판정은 `keep` 195개, `split` 0개, `merge` 0개, `hide` 0개다. 검토 완료 수와 stale source hash는 감사 원장의 현재 summary를 기준으로 하며, `check:storybook-ia`가 이 상태를 회귀 방지한다.
