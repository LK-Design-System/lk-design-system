...
2026-07-18 현재 소스에서 재생성한 정적 빌드 기준으로 191개 페이지와 583개 스토리가 있다. census 판정은 모두 `keep`다.

- 공개 스토리: 447개
- 숨김 스토리: 136개
- 숨김 visual parity: 99개
- 숨김 internal contract: 37개
- 검토 완료 페이지 177/191개, 스토리 534/583개

인간 disposition 검토는 직전 177개 페이지 / 534개 스토리 기준선까지 완료돼 있다. 그 이후 Robotics Foundation 원자화와 Navigation 페이지 재분해, 그리고 Robot Marker 신설과 방향·진행 어휘 재설계로 추가·변경된 페이지(신규 14개 페이지·48개 스토리, 변경 다수)는 다음 `check:storybook-ia` 검토 패스를 기다리며, 그 전까지 아래 레이어별 진입 가이드·설명 계약 수치는 마지막 완료 검토(534개 스토리 / 공개 408개) 기준이다.
...
최종 구조에서는 분리된 각 페이지가 독립 소유자를 가지므로 census 판정은 `keep` 190개, `split` 0개, `merge` 0개, `hide` 0개다. 직전 검토 패스는 177개 페이지·534개 스토리를 완료했고 공개·숨김 권고 불일치는 0개였다. 이후 Robotics Foundation 원자화와 Navigation 재분해로 추가된 13개 페이지·44개 스토리와 변경된 다수 페이지는 아직 재검토 전이며, `check:storybook-ia`가 이 재검토를 회귀 방지한다. Communication 2페이지와 Navigation 5페이지는 직전 검토에서 source hash, 공개 이름, 역할, 표적 play/Axe 결과로 재검토했다.
