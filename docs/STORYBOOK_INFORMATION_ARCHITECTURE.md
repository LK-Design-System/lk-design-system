...
2026-07-17 현재 소스에서 재생성한 정적 빌드 기준으로 190개 페이지와 578개 스토리가 있다. census 판정은 모두 `keep`다.

- 공개 스토리: 442개
- 숨김 스토리: 136개
- 숨김 visual parity: 99개
- 숨김 internal contract: 37개
- 검토 완료 페이지 177/190개, 스토리 534/578개

인간 disposition 검토는 직전 177개 페이지 / 534개 스토리 기준선까지 완료돼 있다. 그 이후 Robotics Foundation 원자화와 Navigation 페이지 재분해로 추가·변경된 페이지(신규 13개 페이지·44개 스토리, 변경 다수)는 다음 `check:storybook-ia` 검토 패스를 기다리며, 그 전까지 아래 레이어별 진입 가이드·설명 계약 수치는 마지막 완료 검토(534개 스토리 / 공개 408개) 기준이다.
