...
- **배지는 예외 상태 전용**: 기본 활동 상태(`active`)는 상태 배지를 그리지 않는다 — solid 강조 선·진행 head·로봇이 이미 "이동 중"을 전달하므로 배지를 더하면 정보 없는 클러터가 된다. 계획·대기·차단·재계산·완료·미확인·오류·충돌·지연 등 **예외 상태만** 배지를 받고, 접근성 이름은 상태와 무관하게 항상 상태를 포함한다. `active` 글리프 자산은 목록·패널·범례 등 비지도 컨텍스트에 남는다.
...
- **로봇이 진행 tip을 점유하면 head는 로봇에게 양보한다.** `RobotMarker`가 경로/궤적의 현재 진행 위치에 놓이는 합성에서는 진행 화살촉과 로봇이 같은 정보(현재 위치)를 두 번 그리게 되므로, 합성이 `showProgressHead={false}`로 head를 생략한다. strong/recessed 진행 대비는 유지되고 화살촉·shaft setback·future gap만 빠지며, 로봇 body가 elapsed→future 접점을 캡핑한다. 로봇 레이어가 꺼지면 head가 되돌아온다.
