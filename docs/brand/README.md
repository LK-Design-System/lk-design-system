...
- 모브랜드 우선 `LK + 제품명`은 live text로 직접 조판하지 않고 승인 SemiBold outline registry인 `ProductLockup`을 사용합니다. 현재 `console`과 `portal`만 지원하며 Web Viz와 Control은 이름·outline 승인 전까지 미지원입니다. 기존 ExtraBold 800 Portal 고정 자산은 정본 변경 판단 전까지 별도 호환·비교 자산으로 유지합니다.
