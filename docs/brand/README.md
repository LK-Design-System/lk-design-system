...
| [`LK_PRODUCT_LOCKUP_STANDARD.md`](LK_PRODUCT_LOCKUP_STANDARD.md) | Portal형 LK mark + 승인 제품명 outline registry, 작도·full/compact·색·접근성·소유권 표준 |
...
- Portal형 `LK + 제품명`은 live text로 직접 조판하지 않고 승인 outline registry인 `ProductLockup`을 사용합니다. 현재 `console`과 `portal`만 지원하며 Web Viz와 Control은 이름·outline 승인 전까지 미지원입니다.
