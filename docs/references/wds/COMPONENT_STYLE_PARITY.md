...
- **DropdownMenu density and width** — WDS `Menu/Menu` keeps a 16px container radius and its
  ...
  available as a compatibility override, while new LDS usage selects semantic
  `density`; fixed width is an explicit layout exception.
...
- **StatusBadge** (`content/StatusBadge.jsx`) — lifecycle/result states now use a **soft
  semantic pill + label**, h20 / padX8 / fs12. The older neutral r4 + coloured-dot
  mapping is retained only as historical WDS snapshot evidence and is superseded by
  current WDS usage evidence.
- **StatusIndicator** (`content/StatusIndicator.jsx`) — the former dot + label anatomy
  is separated for live availability, connection, and freshness only. Optional pulse
  remains explicit and reduced-motion safe.
...
- **Style-verified (real WDS↔LDS diff, 0 drift): ~25 components.**
  - `check:component-styles` — token-driven (node), incl. Button/Input/Chip/Textfield/Menu.
  - `check:component-styles-rendered` — Playwright, **23 components** (adds Input, Divider,
    and Badge; StatusBadge is now an intentional semantic redesign).
...
A batch-2 auto-diff attempt (Tabs, the historical StatusBadge mapping, Input font) surfaced **only false
positives, 0 real drift**: the harness measured the 2px Tab indicator instead of the
48px tab; current StatusBadge usage supersedes the old r4 snapshot and is documented as
an intentional semantic pill while live dots move to `StatusIndicator`; the Input
"font 17" was the field row's inherited
