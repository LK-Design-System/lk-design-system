...
| Audit snapshot | `d0c64ca12a71a00a751c6c12ed6254861eb53a1c` (`feat/robot-marker-pose`; clean-main baseline 아님) |
...
| Robotics | 32 | 44 | Robotics → Core 31건, Product 1건 |
| Aggregate | 205 | 226 total | 네 layer export의 정확한 합집합 |
...
- npm과 pnpm lockfile이 함께 있고 CI 설치 정책도 하나로 고정되지 않았다.
...
`PUBLIC_EXPORT_CLASSIFICATION.json`과 각 prompt는 이들을 포함한 Robotics 32개 public
...
| 2D spatial UI | `RobotMarker`, `RouteOverlay`, `LaneOverlay`, `SpatialRegion` | SVG/DOM 표현과 LDS3D spatial contract의 seam이 명확한가 |

현재 public source module 32개와 Robotics internal module 12개는 모두
...
뜻이 아니라, 실제 좌표·scene·renderer 책임이 LDS3D에 있다는 뜻이다. 현재 226개 LDS
module과 380개 asset 중 LDS3D로 이동할 항목은 없다. DOM/SVG와 WebGL 구현을 이름만
보고 합치지 않는다.
예를 들어 2D `RobotMarker`와 3D robot primitive는 렌더링 매체별 구현을 유지하며,
...
- [x] 현재 205개 public source module, 211개 public symbol과 총 226개 module의 target
      package를 기록한다.
- [x] Robotics 32개 public source module을 앞 절의 네 묶음으로 재검토하고, 현재
...
- `scripts/check-package-migration.mjs`가 repository-owned module target digest, mixed
  ...
  검증한다. 여섯 product와 LDS3D의 current-state scanner report는 Wave 0 evidence로
  고정했으며, versioned artifact portability 검증은 Wave 2에서 수행한다.
...
- LDS3D docs의 reproducible current-state scan은 mutable sibling `link:`, aggregate root
...
- 일곱 consumer evidence report는 이 변경에서 Git 추적 상태로 고정해 consumer evidence
  blocker를 닫았다. 다만 변경 자체는 아직 `feat/robot-marker-pose`에 있고, clean main/tag,
  accountable person, package/CJS/support 승인, full regression evidence, immutable
...
1. 현재 main에는 layer entrypoint와 ownership schema가 없으므로 Wave 0 변경만 단독
   이식하지 않는다. `c20ff3c`의 layer infrastructure를 RobotMarker 시각 변경과 분리해
   검증한 뒤 이 Wave 0 변경과 함께 main에 통합한다.
2. package별 accountable person, editor/viewer Product 후보의 처리와 CJS 최종 정책을
   승인한다.
3. 현재 feature branch 변경의 main 통합 방식을 정한 뒤, clean main에서 legacy
   `pnpm-lock.yaml`을 제거하고 npm frozen install을 재현한다.
