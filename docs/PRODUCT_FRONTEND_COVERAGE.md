...
- 제품 identity의 full 시각 자산은 `ProductLockup` 승인 레지스트리가 소유한다. 초기 key는 `console`, `portal`뿐이며 둘 다 LK가 먼저 읽히는 SemiBold 600 outlined path다. 기존 ExtraBold 800 Portal은 정본 변경 판단을 위한 비교 자산으로 남긴다. 자유 문자열로 LK+제품명을 합성하지 않고, collapsed rail은 같은 승인 key의 `ProductLockup compact`로 명시 전환해 full과 같은 `LK {제품명}` 접근성 이름을 유지한다. 제품은 canonical short lockup name의 제안·승인, identity 소유 region, home route·click과 breakpoint를 소유한다.
...
| `portal` | `546f11e4640b5a9ec81fc210a5085581b9d277bc` | composition compatible, visual review and package upgrade required | 기존 mark+Portal 구조는 호환되지만 새 `ProductLockup product="portal"`은 제품명을 ExtraBold 800에서 SemiBold 600으로 낮춘 제안안이다. Storybook 비교 후 Portal 정본 변경을 승인하고 ProductLockup constructionVersion 2·brand constructionVersion 4 포함 릴리스로 올린다. collapsed rail은 `compact`의 승인 mark로 전환한다. |
