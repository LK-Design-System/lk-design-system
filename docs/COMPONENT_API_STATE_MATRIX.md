...
| ProductLockup | required approved `product: "console" | "portal"`, `appearance: "positive" | "reverse"`, default 28/minimum 20 `height`, explicit `compact`, canonical `aria-label`, decorative host composition | parent-brand-first SVG outlines generated from Montserrat SemiBold 600 v7.222 uppercase with default kerning; visible name height `1X`; visual gap `0.35 ×` visible mark width (not `0.35X`); fixed Portal and registry Portal share canonical paths; no runtime font/raw-name fallback; full/compact accessible-name parity; Web Viz/Control pending unsupported registry names |
...
| AlarmCaseBanner | `lifecycle` (`active·acknowledged·shelved·escalated·cleared`), `severity` (`critical·warning·notice`), required `title`/`target`, `location`, `occurredAt`/`occurredLabel`, `reference`, independent `stale`/`link`/`authority`/`relatedCount` axes, `evidence` (`actor·at·reason·authority`) with `data-evidence="recorded|partial|missing"`, `onAcknowledge` + `acknowledgePending` + `acknowledgeBlockedReason`, `actions`, labelled `remoteAction`, `announce`, `headingLevel`; no lifecycle transition, persistence, siren or remote transport ownership | active/acknowledged/missing-record overview, reason-dialog interaction with remote resume kept separate, view-only/offline/stale/flood/shelved/escalated/cleared variants, 320px list, visual parity |
| DotMatrixPreview | fixed `columns × rows` geometry, 1-bit `bitmap` (MSB-first) or `pixels`, `color` hex and controller `brightness` 0–255 scaling the lit dots, required `label`, `description` text alternative, `status` slot, `dotShape`, `emptyLabel` with `data-empty`; no rasterisation, transport or polling ownership | two-panel overview with brightness scaling, editor composition with empty/disabled apply, 320px + offline empty frame, visual parity |
| ScheduleCalendar | controlled/uncontrolled `view` (`month \| week \| day`, reducible via `views`) and anchor `date`; expanded `events` with `allDay`/`tone`; `onSlotSelect` (day or `slotMinutes` hour range), `onEventActivate`, `onOverflowActivate`; `minTime`/`maxTime`, `nowIndicator` minute tick with cleanup, named parts/vars/ref; no recurrence expansion, persistence, drag editing or form ownership | month overview with +N개 overflow, week overlap column packing, 360px day view with view switch, controlled views subset and time range |
...
| EquipmentStatusCard | required equipment identity and visible primary condition, optional decorative icon/description, semantic tone, optional prominent `readout` with `readoutLabel`/`readoutTone` read after status and before facts, labeled supporting facts in a description list, freshness/ownership metadata and composed actions; noninteractive article with no product transport or equipment state machine | equipment identity/status overview, connection composition, normal width, 300px long-content dark and hidden parity stories |
