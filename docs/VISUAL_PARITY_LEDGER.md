...
| P0 | Navigation: SideNav, Tabs, Breadcrumb, Pagination, BottomNav, Steps, UserMenu | Watch | 원본 카드 대비 spacing, 선택 상태, 다크 대비 검증 |
...
| P1 | Forms: AutoComplete, DatePicker, SearchField, Slider, Input 계열 | Watch | focus ring, error/help text, dark input surface 검증 |
| P1 | Data: Table, Calendar, AvatarGroup | Watch | 행 높이, sticky header, empty/loading 상태 보강 |
| P1 | Overlay: Modal, Drawer, Sheet, Popover, DropdownMenu, Toast, Alert | Watch | focus trap, escape/overlay close, dark surface 대비 검증 |
| P1 | Selection: FilterChip, MultiSelectChip, SegmentedControl, Switch, ToggleButton, ThemeToggle, Stepper | Watch | selected/pressed/disabled 상태의 토큰 일관성 검증 |
| P2 | Cards: ProductCard, NewsCard, FeatureCard, MetricCard, Stat | Watch | 카드 hover movement와 이미지 scale을 제품 톤에 맞게 재판정 |
| P2 | Content: Accordion, ListCell, Tooltip, Badge, Timeline, Divider | Watch | 음수 자간, hover/focus, 말줄임/줄바꿈 검증 |
...
- 일부 비버튼 컴포넌트에는 기존 음수 자간이 남아 있습니다. 버튼과 Footer는 이번에 보정했고, 다음 라운드에서 forms, overlay, content, navigation 세부 컴포넌트를 같은 기준으로 정리해야 합니다.
