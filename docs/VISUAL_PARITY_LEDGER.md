...
| P1 | Visual inventory | Playwright PNG inventory plus card-to-story traceability for all 83 original component cards and every React implementation story |
| P1 | Visual review report | Local side-by-side HTML report for original component card screenshots and paired React story screenshots |
...
- `npm run check:visual-inventory` captures all 83 original component cards and every React implementation story into `visual-artifacts/inventory/` with a manifest and card/story pairing candidates, primary story selections, and local review anchors. This is evidence collection before pixel diff baseline enforcement.
- `npm run check:visual-review` refreshes that inventory and writes `visual-artifacts/inventory/review.html`, a local side-by-side QA report for each original card and its paired React story screenshots with primary-story badges, then verifies that the report renders 83 pairs with no broken images.
