...
- Never add a colored bar or stripe on the leading edge of a row, card, chip, alarm or callout (a 2px+ left/inline-start border, a thicker left border, an inset stripe). This includes mock-ups. Reuse the sibling that owns the job instead: Banner's tonal icon, StatusBadge's tonal surface, SideNav's selected text. `npm run check:no-leading-bars` enforces this; the rule lives in `docs/TOKEN_GOVERNANCE.md`.
